...
- shows bounded recommended, available, and unavailable next-step actions for one resolved target, optionally shaped by one bootstrap host profile preset, without implying execution, installer, or runtime-routing behavior
...
- `plan <target_ref> [--host <host_profile_id>] [--store-root <path>]`
