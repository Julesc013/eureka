...
- renders compatibility-first synchronous resolution-run listing at `/runs?run_store_root=...`, bounded run detail at `/run?id=...&run_store_root=...`, and bounded run-start pages at `/run/resolve?...&run_store_root=...` plus `/run/search?...&run_store_root=...` without implying worker queues, streaming updates, or query planning
...
- exposes `/api/runs?run_store_root=...`, `/api/run?id=...&run_store_root=...`, `/api/run/resolve?target_ref=...&run_store_root=...`, and `/api/run/search?q=...&run_store_root=...` for machine-readable bounded synchronous resolution runs over the same transport-neutral public boundary already reused by the HTML workbench and CLI
...
- resolution-run pages are investigation records only; they must not imply async progression, streaming partial results, or planner-owned reasoning yet
