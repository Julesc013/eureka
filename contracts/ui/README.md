...
- `QueryPlan` for deterministic bounded raw-query compilation into a structured `ResolutionTask`, including compact constraints, prefer/exclude hints, action hints, source hints, confidence, and planner notes without implying LLM planning, vector search, fuzzy retrieval, or ranking
- `LocalIndex` for bootstrap local SQLite index build, status, and query projection, including compact result summaries, source ids and families, record-kind counts, and FTS mode without implying ranking, fuzzy retrieval, vector search, live sync, or incremental indexing
- `LocalTask` for synchronous bootstrap local task creation, persistence, listing, and inspection, including compact requested-input, result-summary, error-summary, and output-reference projection without implying async orchestration, retries, priorities, or distributed queue behavior
- `ResolutionRun` for synchronous local exact-resolution, deterministic-search, and planned-search investigation records, including checked sources plus current result or absence summaries and optional `resolution_task` summaries without implying worker queues, streaming phases, or full investigation-planning semantics
- `ResolutionMemory` for explicit local resolution-memory creation from persisted runs, including compact checked-source summaries, reusable result or absence summaries, useful source ids, evidence summaries, and invalidation hints without implying cloud memory, personalization, ranking, or an invalidation engine
...
This shared-contract layer now carries the first bounded provenance and evidence seam, the first bounded comparison and disagreement seam, the first bounded object/state timeline seam, the first bounded absence-reasoning seam, the first bounded representation/access-path seam, the first bounded compatibility seam, the first bounded action-routing seam, the first bounded user-strategy seam, the first bounded representation-selection and handoff seam, the first bounded acquisition and fetch seam, the first bounded decomposition and package-member seam, the first bounded member-readback and preview seam, the first bounded resolution-run seam, the first bounded deterministic query-planner seam, the first bounded local-index seam, the first bounded local-task seam, and the first bounded local resolution-memory seam. It is not a final provenance graph, trust model, object identity model, representation, acquisition, decomposition, member-extraction, action model, handoff or runtime-launch model, merge contract, compatibility oracle, truth-selection engine, worker queue, full investigation planner, personalization system, cloud-memory system, invalidation engine, diagnostic engine, extraction framework, execution policy engine, or final ranking and retrieval engine.
