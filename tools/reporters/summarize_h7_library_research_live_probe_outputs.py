#!/usr/bin/env python3
"""Summarize H7 library/research live-probe outputs without network use."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        inputs = args.input or ["examples/connectors/h7_library_research/live_probe_results"]
        ...
        if args.json:
            ...
            print("H7 library/research live probe output summary", file=stdout)
...
def load_results(inputs: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for item in inputs:
        ...
        for file in files:
            ...
            if isinstance(payload, Mapping) and payload.get("schema_version") == "h7_library_research_live_probe_result.v0":
...
def summarize(results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "h7_library_research_live_probe_output_summary.v0",
        "status": "valid",
        ...
        "bibliographic_candidate_count": sum(1 for item in results if _candidate_present(item.get("bibliographic_identity_candidate"))),
        "research_work_candidate_count": sum(1 for item in results if _candidate_present(item.get("research_work_identity_candidate"))),
        "dataset_candidate_count": sum(1 for item in results if _candidate_present(item.get("dataset_identity_candidate"))),
        "cultural_object_candidate_count": sum(1 for item in results if _candidate_present(item.get("cultural_object_identity_candidate"))),
        "patent_candidate_count": sum(1 for item in results if _candidate_present(item.get("patent_identity_candidate"))),
        "citation_relation_candidate_count": sum(1 for item in results if item.get("citation_relation_candidate") and not _blocked(item.get("citation_relation_candidate"))),
        "access_rights_candidate_count": sum(1 for item in results if _candidate_present(item.get("access_rights_availability_candidate"))),
...
def render_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        "# H7 Library Research Live Probe Output Summary",
        "",
