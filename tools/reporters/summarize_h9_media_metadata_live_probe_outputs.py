#!/usr/bin/env python3
"""Summarize H9 media metadata live-probe outputs without network use."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        inputs = args.input or ["examples/connectors/h9_media_metadata/live_probe_results"]
        ...
        if args.json:
            ...
            print("H9 media metadata live probe output summary", file=stdout)
...
def load_results(inputs: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for item in inputs:
        ...
        for file in files:
            ...
            if isinstance(payload, Mapping) and payload.get("schema_version") == "h9_media_metadata_live_probe_result.v0":
...
def summarize(results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "h9_media_metadata_live_probe_output_summary.v0",
        "status": "valid",
        ...
        "media_object_candidate_count": sum(1 for item in results if _candidate_present(item.get("media_object_identity_candidate"))),
        "music_work_recording_release_candidate_count": sum(1 for item in results if _candidate_present(item.get("music_work_recording_release_candidate"))),
        "image_video_map_candidate_count": sum(1 for item in results if _candidate_present(item.get("image_video_map_identity_candidate"))),
        "creator_collection_relation_candidate_count": sum(1 for item in results if item.get("media_creator_collection_relation_candidate") and not _blocked(item.get("media_creator_collection_relation_candidate"))),
        "fingerprint_candidate_count": sum(1 for item in results if _candidate_present(item.get("media_fingerprint_candidate"))),
        "rights_license_candidate_count": sum(1 for item in results if _candidate_present(item.get("media_rights_license_candidate"))),
        "safety_privacy_candidate_count": sum(1 for item in results if _candidate_present(item.get("media_safety_privacy_candidate"))),
...
def render_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        "# H9 Media Metadata Live Probe Output Summary",
        "",
