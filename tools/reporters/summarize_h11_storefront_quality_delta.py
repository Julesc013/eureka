#!/usr/bin/env python3
"""Summarize explicit H11 storefront review integration outputs."""
...
from archive.prototypes.legacy_runtime.connectors.h11_storefront.quality_delta import (  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", action="append", default=[], help="Explicit H11 integration JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H11 integration JSON files. May be repeated.")
    ...
    try:
        ...
        summary = summarize_h11_quality_delta(delta)
        ...
        if args.json:
            ...
            print("H11 storefront quality delta", file=stdout)
...
def build_quality_delta(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    review = next((payload for payload in payloads if payload.get("schema_version") == "h11_storefront_review_integration_result.v0"), None)
    if review is None:
        delta = next((payload for payload in payloads if payload.get("schema_version") == "h11_storefront_quality_delta_report.v0"), None)
        if delta is None:
            raise ValueError("no h11_storefront_review_integration_result.v0 input found")
        return dict(delta)
    delta = build_h11_quality_delta({"review_integration_result": review})
    errors = detect_h11_quality_overclaim(delta)
    if errors:
...
def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        ...
        "- review_correctness: `false`",
        "- rating_correctness: `false`",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("examples/connectors/h11_storefront/review_integration/"):
            return resolved
