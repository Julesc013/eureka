#!/usr/bin/env python3
"""Summarize explicit H12 retro/community review integration outputs."""
...
from archive.prototypes.legacy_runtime.connectors.h12_retro_community.quality_delta import (  # noqa: E402
    build_h12_quality_delta,
    detect_h12_quality_overclaim,
    summarize_h12_quality_delta,
)


def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", action="append", default=[], help="Explicit H12 integration JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H12 integration JSON files. May be repeated.")
    ...
    try:
        ...
        summary = summarize_h12_quality_delta(delta)
        ...
        if args.json:
            ...
            print("H12 retro/community quality delta", file=stdout)
...
def build_quality_delta(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    review = next((payload for payload in payloads if payload.get("schema_version") == "h12_retro_community_review_integration_result.v0"), None)
    if review is None:
        delta = next((payload for payload in payloads if payload.get("schema_version") == "h12_retro_community_quality_delta_report.v0"), None)
        if delta is None:
            raise ValueError("no h12_retro_community_review_integration_result.v0 input found")
        return dict(delta)
    delta = build_h12_quality_delta({"review_integration_result": review})
    errors = detect_h12_quality_overclaim(delta)
    if errors:
...
def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        ...
        "- retro_software_identity_verified: `false`",
        "- platform_version_verified: `false`",
        "- archive_item_member_verified: `false`",
        "- file_authenticity_verified: `false`",
        "- checksum_correctness_verified: `false`",
        "- compatibility_correctness: `false`",
        "- installability: `false`",
        "- playability: `false`",
        "- legal_acquisition: `false`",
        "- rights_clearance: `false`",
        "- malware_safety: `false`",
        "- content_safety: `false`",
        "- privacy_safety: `false`",
        "- community_reputation: `false`",
        "",
    ])
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("examples/connectors/h12_retro_community/review_integration/"):
            return resolved
