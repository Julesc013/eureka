#!/usr/bin/env python3
"""Summarize H3 OS package archive sources and policy-pack posture offline."""
...
SOURCE_INVENTORY = REPO_ROOT / "control/inventory/source_packs/h3_os_package_archive_sources.json"
CONNECTOR_FAMILIES = REPO_ROOT / "control/inventory/source_packs/h3_os_package_archive_connector_families.json"
IDENTITY_POLICY = REPO_ROOT / "control/inventory/source_packs/h3_os_package_identity_policy.json"
COMPATIBILITY_POLICY = REPO_ROOT / "control/inventory/source_packs/h3_os_platform_compatibility_policy.json"
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("H3 OS package archive source summary", file=stdout)
            ...
            print(f"repository_index_fetch_enabled_count: {summary['repository_index_fetch_enabled_count']}", file=stdout)
            print(f"package_download_enabled_count: {summary['package_download_enabled_count']}", file=stdout)
        return 0
...
def build_summary() -> dict[str, Any]:
    ...
    identity_policy = _load_json(IDENTITY_POLICY)
    compatibility_policy = _load_json(COMPATIBILITY_POLICY)
    sources = inventory.get("sources", [])
    if not isinstance(sources, list):
        raise ValueError("H3 source inventory must contain a sources list")
    ...
    identity_count = sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("os_package_identity_support"), Mapping))
    compatibility_count = sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("os_platform_compatibility_support"), Mapping))
    historical_snapshot_count = sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("historical_snapshot_support"), Mapping))
    ...
    repository_index_fetch_enabled_count = sum(1 for item in sources if isinstance(item, Mapping) and item.get("repository_index_fetch_enabled") is True)
    package_download_enabled_count = sum(1 for item in sources if isinstance(item, Mapping) and item.get("package_download_enabled") is True)
    package_manager_invocation_enabled_count = sum(1 for item in sources if isinstance(item, Mapping) and item.get("package_manager_invocation_enabled") is True)
    install_execute_enabled_count = sum(1 for item in sources if isinstance(item, Mapping) and item.get("install_execute_enabled") is True)
    return {
        "schema_version": "h3_os_package_archive_source_summary.v0",
        "status": "pass",
        "wave_id": inventory.get("wave_id", "H3"),
        ...
        "os_package_identity_support_count": identity_count,
        "os_platform_compatibility_support_count": compatibility_count,
        "historical_snapshot_support_count": historical_snapshot_count,
        "identity_concepts": identity_policy.get("identity_concepts", []),
        ...
        "repository_index_fetch_enabled_count": repository_index_fetch_enabled_count,
        "package_download_enabled_count": package_download_enabled_count,
        "package_manager_invocation_enabled_count": package_manager_invocation_enabled_count,
        "install_execute_enabled_count": install_execute_enabled_count,
        "blockers": [
            "fixture runtime not implemented",
            "live access not approved",
            "repository index fetch forbidden",
            "package downloads forbidden",
            "package-manager invocation forbidden",
            "public and master index mutation forbidden",
        ],
        "readiness": "READY_FOR_H3_FIXTURE_RUNTIME",
        "truth_boundary": {
...
def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    lines = [
        "# H3 OS Package Archive Source Summary",
        "",
        ...
        f"- repository_index_fetch_enabled_count: `{summary.get('repository_index_fetch_enabled_count', 0)}`",
        f"- package_download_enabled_count: `{summary.get('package_download_enabled_count', 0)}`",
        f"- package_manager_invocation_enabled_count: `{summary.get('package_manager_invocation_enabled_count', 0)}`",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/source_packs/") or rel_lower.startswith("examples/connectors/h3_os_package_archives/"):
            return resolved
        raise ValueError(f"refusing output outside approved H3 roots: {rel}")
