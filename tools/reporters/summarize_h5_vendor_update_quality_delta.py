#!/usr/bin/env python3
"""Summarize explicit H5 vendor/update/driver review integration outputs."""
...
from archive.prototypes.legacy_runtime.connectors.h5_vendor_update_driver.quality_delta import (  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", action="append", default=[], help="Explicit H5 integration JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H5 integration JSON files. May be repeated.")
    ...
    try:
        ...
        summary = summarize_h5_quality_delta(delta)
        ...
        if args.json:
            ...
            print("H5 vendor/update/driver quality delta", file=stdout)
...
def build_quality_delta(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    review = next((payload for payload in payloads if payload.get("schema_version") == "h5_vendor_update_review_integration_result.v0"), None)
    if review is None:
        raise ValueError("no h5_vendor_update_review_integration_result.v0 input found")
    delta = build_h5_quality_delta({"review_integration_result": review})
    errors = detect_h5_quality_overclaim(delta)
    if errors:
