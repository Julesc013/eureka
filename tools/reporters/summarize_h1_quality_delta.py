#!/usr/bin/env python3
"""Build and summarize H1 metadata-wave quality delta reports offline."""
...
from archive.prototypes.legacy_runtime.connectors.h1_metadata_wave.quality_delta import (  # noqa: E402
    build_h1_quality_delta,
    detect_h1_quality_overclaim,
    summarize_h1_quality_delta,
)
from archive.prototypes.legacy_runtime.connectors.h1_metadata_wave.wave_postmortem import (  # noqa: E402
    build_h1_connector_wave_postmortem,
    build_h1_next_phase_recommendation,
)
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", action="append", default=[], help="Explicit H1 integration JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H1 integration JSON files. May be repeated.")
    ...
    try:
        ...
        if args.json:
            ...
            print("H1 quality delta", file=stdout)
...
def run_quality_delta(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    delta = build_h1_quality_delta({"review_integration_result": review_result})
    postmortem = build_h1_connector_wave_postmortem(review_result, delta)
    recommendation = build_h1_next_phase_recommendation(postmortem)
    errors = detect_h1_quality_overclaim(delta) + detect_h1_quality_overclaim(postmortem) + detect_h1_quality_overclaim(recommendation)
    if errors:
        raise ValueError("; ".join(errors))
    summary = summarize_h1_quality_delta(delta)
    return {
...
def load_review_integration_result(inputs: Sequence[str], input_dirs: Sequence[str]) -> Mapping[str, Any]:
    ...
    for path in paths:
        ...
        if isinstance(payload, Mapping) and payload.get("schema_version") == "h1_review_integration_result.v0":
            return dict(payload)
    raise ValueError("no h1_review_integration_result.v0 JSON found in inputs")


def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            "# H1 Quality Delta Summary",
            "",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        raise ValueError(f"refusing output outside approved H1 quality roots: {rel_path}")
