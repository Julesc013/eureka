#!/usr/bin/env python3
"""Summarize H13 local/private review quality delta offline."""
...
from archive.prototypes.legacy_runtime.connectors.h13_local_private.quality_delta import build_h13_quality_delta, summarize_h13_quality_delta  # noqa: E402

FORBIDDEN_OUTPUT_ROOTS = (
    "site/dist", "site/dist/data/public_index", "runtime", "contracts", "control/inventory/sources",
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", action="append", default=[], help="Explicit H13 integration output JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H13 integration output JSON files. May be repeated.")
    ...
    try:
        ...
        summary = summarize_h13_quality_delta(delta)
        wrote = False
        ...
        if args.json:
            ...
            print("H13 local/private quality delta", file=stdout)
...
def _load_review_result(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    for item in objects:
        if item.get("schema_version") == "h13_local_private_review_integration_result.v0":
            return item
    for item in objects:
        if "local_source_identity_review_seeds" in item:
            return item
    raise ValueError("no H13 review integration result found")


def render_summary(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        "# H13 Quality Delta Summary",
        "",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("examples/connectors/h13_local_private/review_integration/") or rel_lower.startswith("control/audits/"):
            return resolved
        raise ValueError(f"refusing output outside approved H13 review roots: {rel}")
