#!/usr/bin/env python3
"""Summarize H11 storefront live-probe outputs without network use."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        inputs = args.input or ["examples/connectors/h11_storefront/live_probe_results"]
        ...
        if args.json:
            ...
            print("H11 storefront live probe output summary", file=stdout)
...
def load_results(inputs: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for item in inputs:
        ...
        for file in files:
            ...
            if isinstance(payload, Mapping) and payload.get("schema_version") == "h11_storefront_live_probe_result.v0":
...
def summarize(results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "h11_storefront_live_probe_output_summary.v0",
        "status": "valid",
        ...
        "storefront_listing_identity_candidate_count": sum(1 for item in results if _candidate_present(item.get("storefront_listing_identity_candidate"))),
        "app_product_identity_candidate_count": sum(1 for item in results if _candidate_present(item.get("app_product_identity_candidate"))),
        "version_release_channel_candidate_count": sum(1 for item in results if _candidate_present(item.get("version_release_channel_candidate"))),
        "price_availability_region_candidate_count": sum(1 for item in results if _candidate_present(item.get("price_availability_region_candidate"))),
        "acquisition_path_candidate_count": sum(1 for item in results if _candidate_present(item.get("acquisition_path_candidate"))),
        "review_rating_metadata_candidate_count": sum(1 for item in results if _candidate_present(item.get("review_rating_metadata_candidate"))),
        "account_entitlement_boundary_candidate_count": sum(1 for item in results if _candidate_present(item.get("account_entitlement_boundary_candidate"))),
        "storefront_rights_safety_candidate_count": sum(1 for item in results if _candidate_present(item.get("storefront_rights_safety_candidate"))),
...
def render_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        "# H11 Storefront Live Probe Output Summary",
        "",
