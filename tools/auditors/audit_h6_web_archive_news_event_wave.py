#!/usr/bin/env python3
"""Audit H6 web archive/news/event wave artifacts offline."""
...
from archive.prototypes.legacy_runtime.connectors.h6_web_archive_news_event.normalizer_common import H6_SOURCE_IDS  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if audit["h6_exit_gate"] in {"FAIL", "PARTIAL", "BLOCKED"}:
            status = "partial" if audit["h6_exit_gate"] == "PARTIAL" else "blocked"
        else:
            status = "pass"
        print("H6 web archive/news/event wave audit", file=stdout)
        print(f"status: {status}", file=stdout)
        print(f"h6_exit_gate: {audit['h6_exit_gate']}", file=stdout)
...
def build_wave_audit(root: Path = REPO_ROOT) -> dict[str, Any]:
    errors: list[str] = []
    required_paths = [
        "control/audits/h6-bundle-01-web-archive-news-event-policy-packs-v0/h6_bundle_01_report.json",
        "control/audits/h6-bundle-02-web-archive-fixture-runtime-v0/h6_bundle_02_report.json",
        "control/audits/h6-bundle-03-web-archive-live-probes-v0/h6_bundle_03_report.json",
        "control/audits/h6-bundle-04-web-archive-review-quality-audit-v0/h6_bundle_04_report.json",
        "examples/connectors/h6_web_archive_news_event/replay_results",
        "examples/connectors/h6_web_archive_news_event/live_probe_results",
    ]
    for rel in required_paths:
        if not (root / rel).exists():
            errors.append(f"missing artifact: {rel}")
    replay_paths = sorted((root / "examples/connectors/h6_web_archive_news_event/replay_results").glob("*.json"))
    live_paths = sorted((root / "examples/connectors/h6_web_archive_news_event/live_probe_results").glob("*.json"))
    outputs = load_h6_web_archive_outputs([*replay_paths, *live_paths])
    review = build_h6_review_integration_result({"outputs": outputs, "input_refs": [rel_path(path) for path in [*replay_paths, *live_paths]]})
    ...
    audit = build_h6_integration_audit(review, delta, postmortem, recommendation)
    if set(audit.get("audited_sources", [])) != set(H6_SOURCE_IDS):
        errors.append("audited sources must match all thirteen H6 source IDs")
    errors.extend(detect_h6_review_truth_boundary_violations(audit))
    errors.extend(detect_h6_review_product_boundary_violations(audit))
    if errors:
        audit["h6_exit_gate"] = "PARTIAL"
...
def render_summary(audit: Mapping[str, Any]) -> str:
    return "\n".join([
        "# H6 Integration Audit Summary",
        "",
        f"- h6_exit_gate: `{audit.get('h6_exit_gate')}`",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and (rel_lower.endswith(".json") or rel_lower.endswith(".md")):
            return resolved
        raise ValueError(f"refusing output outside approved H6 audit roots: {rel}")
