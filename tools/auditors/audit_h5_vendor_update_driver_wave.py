#!/usr/bin/env python3
"""Audit H5 vendor/update/driver wave artifacts offline."""
...
from archive.prototypes.legacy_runtime.connectors.h5_vendor_update_driver.normalizer_common import H5_SOURCE_IDS  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if audit["h5_exit_gate"] in {"FAIL", "PARTIAL", "BLOCKED"}:
            status = "partial" if audit["h5_exit_gate"] == "PARTIAL" else "blocked"
        else:
            status = "pass"
        print("H5 vendor/update/driver wave audit", file=stdout)
        print(f"status: {status}", file=stdout)
        print(f"h5_exit_gate: {audit['h5_exit_gate']}", file=stdout)
...
def build_wave_audit(root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    required_paths = [
        ...
        "examples/connectors/h5_vendor_update_driver/replay_results",
        "examples/connectors/h5_vendor_update_driver/live_probe_results",
    ]
    for rel in required_paths:
        if not (root / rel).exists():
            errors.append(f"missing artifact: {rel}")
    replay_paths = sorted((root / "examples/connectors/h5_vendor_update_driver/replay_results").glob("*.json"))
    live_paths = sorted((root / "examples/connectors/h5_vendor_update_driver/live_probe_results").glob("*.json"))
    outputs = load_h5_vendor_update_outputs([*replay_paths, *live_paths])
    review = build_h5_review_integration_result({"outputs": outputs, "input_refs": [rel_path(path) for path in [*replay_paths, *live_paths]]})
    ...
    audit = build_h5_integration_audit(review, delta, postmortem, recommendation)
    if set(audit.get("audited_sources", [])) != set(H5_SOURCE_IDS):
        errors.append("audited sources must match all fifteen H5 source IDs")
    errors.extend(detect_h5_review_truth_boundary_violations(audit))
    errors.extend(detect_h5_review_product_boundary_violations(audit))
    if errors:
        audit["h5_exit_gate"] = "PARTIAL"
...
def render_summary(audit: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            "# H5 Integration Audit Summary",
            "",
            f"- h5_exit_gate: `{audit.get('h5_exit_gate')}`",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and (rel_lower.endswith(".json") or rel_lower.endswith(".md")):
            return resolved
        raise ValueError(f"refusing output outside approved H5 audit roots: {rel}")
