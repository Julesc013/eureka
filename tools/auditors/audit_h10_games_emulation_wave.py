#!/usr/bin/env python3
"""Audit H10 games/emulation wave artifacts offline."""
...
from archive.prototypes.legacy_runtime.connectors.h10_games_emulation.normalizer_common import H10_SOURCE_IDS  # noqa: E402
from archive.prototypes.legacy_runtime.connectors.h10_games_emulation.quality_delta import build_h10_quality_delta  # noqa: E402
from archive.prototypes.legacy_runtime.connectors.h10_games_emulation.review_integration import (  # noqa: E402
...
from archive.prototypes.legacy_runtime.connectors.h10_games_emulation.wave_postmortem import (  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        status = "pass" if audit["h10_exit_gate"] in {"PASS", "PASS_WITH_WARNINGS"} else "partial"
        print("H10 games/emulation wave audit", file=stdout)
        print(f"status: {status}", file=stdout)
        print(f"h10_exit_gate: {audit['h10_exit_gate']}", file=stdout)
...
def build_wave_audit(root: Path = REPO_ROOT) -> dict[str, Any]:
    errors: list[str] = []
    required_paths = [
        "control/audits/h10-bundle-01-games-emulation-policy-packs-v0/h10_bundle_01_report.json",
        "control/audits/h10-bundle-02-games-emulation-fixture-runtime-v0/h10_bundle_02_report.json",
        "control/audits/h10-bundle-03-games-emulation-live-probes-v0/h10_bundle_03_report.json",
        "control/audits/h10-bundle-04-games-emulation-review-quality-audit-v0/h10_bundle_04_report.json",
        "examples/connectors/h10_games_emulation/replay_results",
        "examples/connectors/h10_games_emulation/live_probe_results",
    ]
    for rel in required_paths:
        if not (root / rel).exists():
            errors.append(f"missing artifact: {rel}")
    replay_paths = sorted((root / "examples/connectors/h10_games_emulation/replay_results").glob("*.json"))
    live_paths = sorted((root / "examples/connectors/h10_games_emulation/live_probe_results").glob("*.json"))
    outputs = load_h10_games_emulation_outputs([*replay_paths, *live_paths])
    review = build_h10_review_integration_result({"outputs": outputs, "input_refs": [rel_path(path) for path in [*replay_paths, *live_paths]]})
    h10_live_report = load_json(root / "control/audits/h10-bundle-03-games-emulation-live-probes-v0/h10_bundle_03_report.json")
    report_blocked = h10_live_report.get("live_probe_results", {}).get("blocked_sources", [])
    if isinstance(report_blocked, list):
        ...
        review["warnings"] = ["H10 live probes remain blocked pending operator approval."] if review["blocked_sources"] else []
    ...
    audit = build_h10_integration_audit(review, delta, postmortem, recommendation)
    if set(audit.get("audited_sources", [])) != set(H10_SOURCE_IDS):
        errors.append("audited sources must match all fourteen H10 source IDs")
    errors.extend(detect_h10_review_truth_boundary_violations(audit))
    errors.extend(detect_h10_review_product_boundary_violations(audit))
    if errors:
        audit["h10_exit_gate"] = "PARTIAL"
...
def render_summary(audit: Mapping[str, Any]) -> str:
    return "\n".join([
        "# H10 Integration Audit Summary",
        "",
        f"- h10_exit_gate: `{audit.get('h10_exit_gate')}`",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and (rel_lower.endswith(".json") or rel_lower.endswith(".md")):
            return resolved
        raise ValueError(f"refusing output outside approved H10 audit roots: {rel}")
