...
def build_public_alpha_wsgi_app(config: PublicAlphaWrapperConfig) -> WorkbenchWsgiApp:
    return WorkbenchWsgiApp(
        ...
        public_search_public_api=build_demo_public_search_public_api(
            archive_org_metadata_candidates_enabled=config.allow_archive_org_metadata_candidates,
            archive_org_metadata_candidate_rows=config.max_results_per_source,
            archive_org_metadata_timeout_seconds=max(1, config.source_timeout_ms // 1000),
        ),
...
def format_config_check(summary: dict[str, object]) -> str:
    lines = [
        ...
        f"archive_org_metadata_candidates_enabled: {summary['archive_org_metadata_candidates_enabled']}",
