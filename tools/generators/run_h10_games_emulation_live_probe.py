#!/usr/bin/env python3
"""Run or preflight H10 games/emulation metadata-only live probes with fail-closed gates."""
...
from archive.prototypes.legacy_runtime.connectors.h10_games_emulation.live_probe_common import (  # noqa: E402
    SOURCE_CONFIGS,
    build_h10_games_emulation_live_probe_blocked_result,
    build_h10_games_emulation_live_probe_output_bundle,
    build_h10_games_emulation_live_probe_request,
    load_h10_games_emulation_live_probe_policy_bundle,
    summarize_h10_games_emulation_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if not args.check:
            outputs = {
                args.output: result,
                args.game_output: result["game_software_identity_candidate"],
                args.release_output: result["platform_release_edition_candidate"],
                args.compatibility_output: result["emulator_compatibility_candidate"],
                args.hashset_output: result["preservation_hashset_candidate"],
                args.media_output: result["rom_disc_media_identity_candidate"],
                args.relation_output: result["game_relation_candidate"],
                args.action_output: result["emulator_action_candidate"],
                args.rights_safety_output: result["games_rights_safety_candidate"],
        ...
        summary = {
            ...
            "live_probe": summarize_h10_games_emulation_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H10 games emulation live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    validation = validate_h10_games_emulation_live_probe_request(request, policy_bundle)
    if not validation["approved"]:
        result = build_h10_games_emulation_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h10_games_emulation_live_probe_output_bundle(result)}
    if not live:
        result = build_h10_games_emulation_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h10_games_emulation_live_probe_output_bundle(result)}
    result = build_h10_games_emulation_live_probe_blocked_result(
        request,
        ["live network execution remains fail-closed in H10-BUNDLE-03; source-specific executor requires future reviewed approval"],
        policy_bundle,
    )
...
def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h10_games_emulation_live_probe_result(result)
    lines = [
        "# H10 Games Emulation Live Probe Summary",
        "",
        ...
        "- software_list_hashset_fetch: `false unless approved bounded metadata-only`",
        "- downloads: `false`",
        "- uploads: `false`",
        "- execution: `false`",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h10_games_emulation_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h10_games_emulation/live_probe_results/"):
            return resolved
        if rel_lower.startswith("examples/connectors/h10_games_emulation/live_probe_outputs/"):
            return resolved
        raise ValueError(f"refusing output outside approved H10 live-probe roots: {rel}")
