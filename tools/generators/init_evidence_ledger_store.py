#!/usr/bin/env python3
"""Initialize an explicit SQLite evidence ledger store."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout, stderr: TextIO = sys.stderr) -> int:
    ...
    errors = list(validate_evidence_ledger_path(db_path))
    if errors:
        result = {"schema_version": "evidence_ledger_init_result.v0", "status": "fail", "errors": errors}
    ...
    with EvidenceLedgerStore.open(db_path) as store:
        ...
        result = {
            "schema_version": "evidence_ledger_init_result.v0",
    ...
    if args.json:
        ...
        print(f"evidence ledger store init: {result['status']}", file=stdout)
