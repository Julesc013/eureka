#!/usr/bin/env python3
"""Replay committed H1 metadata-wave fixtures offline."""
...
from archive.prototypes.legacy_runtime.connectors.h1_metadata_wave.fixture_loader import load_h1_fixture  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--source-id", choices=H1_SOURCE_IDS)
    parser.add_argument("--fixture-root", default="examples/connectors/h1_metadata_wave/fixtures")
    ...
    try:
        ...
        summary = {
            "schema_version": "h1_metadata_fixture_replay_summary.v0",
            "status": "pass",
        ...
        if args.json:
            ...
            print("H1 metadata fixture replay", file=stdout)
...
def replay_fixtures(fixture_root: str | Path, source_id: str | None = None) -> list[dict[str, Any]]:
    ...
    source_ids = [source_id] if source_id else list(H1_SOURCE_IDS)
    ...
    for item in source_ids:
        ...
        for fixture_path in sorted(source_dir.glob("*.json")):
            ...
            results.append(build_h1_fixture_replay_result(fixture, normalized))
    return results
...
def _safe_output_dir(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and rel_lower.endswith("/generated"):
            return resolved
        if rel_lower.startswith("examples/connectors/h1_metadata_wave/replay_results"):
            return resolved
        raise ValueError(f"refusing output outside approved H1 replay roots: {rel}")
