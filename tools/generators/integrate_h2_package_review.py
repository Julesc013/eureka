#!/usr/bin/env python3
"""Integrate explicit H2 package outputs into offline review previews."""
...
from archive.prototypes.legacy_runtime.connectors.h2_package_registries.review_integration import (  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", action="append", default=[], help="Explicit H2 output JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H2 output JSON files. May be repeated.")
    ...
    try:
        ...
        summary = summarize_h2_review_integration(artifacts["review_integration_result"])
        summary["wrote_files"] = bool(args.output_dir and not args.check)
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            print("H2 package review integration", file=stdout)
            print(f"status: {summary['status']}", file=stdout)
            print(f"source_count: {summary['source_count']}", file=stdout)
            print(f"package_identity_review_seed_count: {summary['package_identity_review_seed_count']}", file=stdout)
...
def run_integration(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    outputs = load_h2_package_outputs(paths)
    result = build_h2_review_integration_result({"outputs": outputs, "input_refs": [rel(path) for path in paths]})
    errors = detect_h2_review_truth_boundary_violations(result) + detect_h2_review_product_boundary_violations(result)
    if errors:
        raise ValueError("; ".join(errors))
    return {
        "review_integration_result": result,
        "package_identity_review_seed": _first(result.get("package_identity_review_seeds")),
        ...
        "summary_markdown": render_summary_markdown(summarize_h2_review_integration(result)),
    }


def build_blocked_review_integration(result: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h2_blocked_review_integration.v0",
        "wave_id": "H2",
...
def write_outputs(output_dir_text: str, artifacts: Mapping[str, Any]) -> None:
    ...
    files = {
        "h2_package_identity_review_seed_v0.json": artifacts["package_identity_review_seed"],
        "h2_dependency_candidate_review_seed_v0.json": artifacts["dependency_candidate_review_seed"],
        "h2_package_file_candidate_review_seed_v0.json": artifacts["package_file_candidate_review_seed"],
        "h2_source_cache_review_seed_v0.json": artifacts["source_cache_review_seed"],
        "h2_evidence_candidate_review_seed_v0.json": artifacts["evidence_candidate_review_seed"],
        "h2_candidate_promotion_preview_v0.json": artifacts["candidate_promotion_preview"],
        "h2_source_coverage_update_preview_v0.json": artifacts["source_coverage_update_preview"],
        "h2_connector_scorecard_update_v0.json": artifacts["connector_scorecard_update"],
        "h2_source_pack_update_preview_v0.json": artifacts["source_pack_update_preview"],
        "h2_review_integration_result_v0.json": artifacts["review_integration_result"],
        "h2_blocked_review_integration_v0.json": artifacts["blocked_review_integration"],
    }
    for name, payload in files.items():
        (output_dir / name).write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (output_dir / "h2_summary.md").write_text(str(artifacts["summary_markdown"]), encoding="utf-8")
    if output_dir.name == "generated":
        (output_dir / "sample_h2_review_integration_result.json").write_text(
            json.dumps(artifacts["review_integration_result"], indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        (output_dir / "sample_h2_summary.md").write_text(str(artifacts["summary_markdown"]), encoding="utf-8")


def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            "# H2 Review Integration Summary",
            "",
            f"- status: `{summary.get('status')}`",
            f"- source_count: `{summary.get('source_count', 0)}`",
            f"- package_identity_review_seed_count: `{summary.get('package_identity_review_seed_count', 0)}`",
...
def _safe_output_dir(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and rel_lower.endswith("/generated"):
            return resolved
        raise ValueError(f"refusing output outside approved H2 review roots: {rel_path}")
