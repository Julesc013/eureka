#!/usr/bin/env python3
"""Run or preflight H12 retro/community metadata-only live probes with fail-closed gates."""
...
from archive.prototypes.legacy_runtime.connectors.h12_retro_community.live_probe_common import (  # noqa: E402
    SOURCE_CONFIGS,
    build_h12_retro_community_live_probe_blocked_result,
    build_h12_retro_community_live_probe_output_bundle,
    build_h12_retro_community_live_probe_request,
    load_h12_retro_community_live_probe_policy_bundle,
    summarize_h12_retro_community_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if not args.check:
            outputs = {
                args.output: result,
                args.software_output: result["retro_software_identity_candidate"],
                args.platform_output: result["platform_version_edition_candidate"],
                args.archive_output: result["archive_item_member_candidate"],
                args.compatibility_output: result["compatibility_install_note_candidate"],
                args.community_output: result["community_review_comment_candidate"],
                args.hash_output: result["hash_checksum_candidate"],
                args.corroboration_output: result["ia_wayback_corroboration_candidate"],
                args.gated_boundary_output: result["gated_source_boundary_candidate"],
                args.rights_safety_output: result["retro_rights_safety_candidate"],
        ...
        summary = {
            ...
            "wrote_files": (not args.check) and any([
                ...
                args.platform_output,
                args.archive_output,
                args.compatibility_output,
                args.community_output,
                args.hash_output,
                args.corroboration_output,
                args.gated_boundary_output,
            ...
            "live_probe": summarize_h12_retro_community_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H12 retro/community live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    validation = validate_h12_retro_community_live_probe_request(request, policy_bundle)
    if not validation["approved"]:
        result = build_h12_retro_community_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h12_retro_community_live_probe_output_bundle(result)}
    if not live:
        result = build_h12_retro_community_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h12_retro_community_live_probe_output_bundle(result)}
    result = build_h12_retro_community_live_probe_blocked_result(
        request,
        ["live network execution remains fail-closed in H12-BUNDLE-03; source-specific transport requires future reviewed approval"],
        policy_bundle,
    )
...
def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h12_retro_community_live_probe_result(result)
    lines = [
        "# H12 Retro/Community Live Probe Summary",
        "",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h12_retro_community_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
