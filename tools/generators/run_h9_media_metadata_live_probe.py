#!/usr/bin/env python3
"""Run or preflight H9 media metadata live probes with fail-closed policy gates."""
...
from archive.prototypes.legacy_runtime.connectors.h9_media_metadata.live_probe_common import (  # noqa: E402
    SOURCE_CONFIGS,
    ...
    summarize_h9_media_metadata_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if not args.check:
            outputs = {
                args.output: result,
                args.media_output: result["media_object_identity_candidate"],
                args.music_output: result["music_work_recording_release_candidate"],
                args.visual_output: result["image_video_map_identity_candidate"],
                args.relation_output: result["media_creator_collection_relation_candidate"],
                args.fingerprint_output: result["media_fingerprint_candidate"],
                args.rights_output: result["media_rights_license_candidate"],
                args.safety_output: result["media_safety_privacy_candidate"],
        ...
        summary = {
            ...
            "live_probe": summarize_h9_media_metadata_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H9 media metadata live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    validation = validate_h9_media_metadata_live_probe_request(request, policy_bundle)
    if not validation["approved"]:
        result = build_h9_media_metadata_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h9_media_metadata_live_probe_output_bundle(result)}
    if not live:
        result = build_h9_media_metadata_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h9_media_metadata_live_probe_output_bundle(result)}
    result = build_h9_media_metadata_live_probe_blocked_result(
        request,
        ["live network execution remains fail-closed in H9-BUNDLE-03; source-specific executor requires future reviewed approval"],
        policy_bundle,
    )
...
def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h9_media_metadata_live_probe_result(result)
    lines = [
        "# H9 Media Metadata Live Probe Summary",
        "",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h9_media_metadata_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h9_media_metadata/live_probe_results/"):
            return resolved
        if rel_lower.startswith("examples/connectors/h9_media_metadata/live_probe_outputs/"):
            return resolved
        raise ValueError(f"refusing output outside approved H9 live-probe roots: {rel}")
