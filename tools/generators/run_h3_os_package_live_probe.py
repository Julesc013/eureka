#!/usr/bin/env python3
"""Run or preflight H3 OS package metadata live probes with fail-closed policy gates."""
...
from archive.prototypes.legacy_runtime.connectors.h3_os_package_archives.live_probe_common import (  # noqa: E402
    H3OSPackageLiveProbeBlocked,
    SOURCE_CONFIGS,
    build_h3_os_package_live_probe_blocked_result,
    build_h3_os_package_live_probe_output_bundle,
    build_h3_os_package_live_probe_request,
    build_h3_os_package_live_probe_result,
    fetch_h3_os_package_metadata_once,
    load_h3_os_package_live_probe_policy_bundle,
    summarize_h3_os_package_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", help="Optional explicit H3 OS package live-probe request envelope.")
    parser.add_argument("--output")
    parser.add_argument("--identity-output")
    parser.add_argument("--compatibility-output")
    parser.add_argument("--dependency-output")
    parser.add_argument("--file-candidate-output")
    ...
    try:
        ...
        if not args.check:
            ...
            if args.identity_output:
                _write_json(args.identity_output, result["os_package_identity_candidate"])
            if args.compatibility_output:
                _write_json(args.compatibility_output, result["os_platform_compatibility_candidate"])
            if args.dependency_output:
                _write_json(args.dependency_output, {"dependency_candidate_preview": result["dependency_candidate_preview"]})
            if args.file_candidate_output:
                _write_json(args.file_candidate_output, {"package_file_candidate_preview": result["package_file_candidate_preview"]})
        ...
        summary = {
            ...
            "live_probe": summarize_h3_os_package_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H3 OS package metadata live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    ...
    if not validation["approved"]:
        result = build_h3_os_package_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h3_os_package_live_probe_output_bundle(result)}
    if not live:
        result = build_h3_os_package_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h3_os_package_live_probe_output_bundle(result)}
    try:
        payload, metadata = fetch_h3_os_package_metadata_once(request, policy_bundle)
    except H3OSPackageLiveProbeBlocked as exc:
        result = exc.result
        return {"live_probe_result": result, "output_bundle": build_h3_os_package_live_probe_output_bundle(result)}
    result = build_h3_os_package_live_probe_result(str(request["source_id"]), payload, metadata, policy_bundle)
    return {"live_probe_result": result, "output_bundle": build_h3_os_package_live_probe_output_bundle(result)}


def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h3_os_package_live_probe_result(result)
    lines = [
        "# H3 OS Package Metadata Live Probe Summary",
        "",
        ...
        f"- repository_index_sync_used: `{str(summary['repository_index_sync_used']).lower()}`",
        f"- dependency_candidate_count: `{summary['dependency_candidate_count']}`",
        f"- file_candidate_count: `{summary['file_candidate_count']}`",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h3_os_package_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h3_os_package_archives/live_probe_results/"):
            return resolved
        if rel_lower.startswith("examples/connectors/h3_os_package_archives/live_probe_outputs/"):
            return resolved
        raise ValueError(f"refusing output outside approved H3 live-probe roots: {rel}")
