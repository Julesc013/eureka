#!/usr/bin/env python3
"""Record a local-only Search Miss Ledger report from explicit JSON input.

The command writes no files by default. It performs no network calls, no model
calls, no public telemetry, no public search mutation, and no master-index
mutation.
"""
...
from runtime.local.foundry.search_miss_ledger import (  # noqa: E402
...
def build_report(input_path: Path, *, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    record = build_search_miss_from_query_observation(payload)
    ...
    return {
        ...
        "truth_boundary": {
            ...
            "search_miss_is_exhaustive_global_absence": False,
...
def main(argv: Sequence[str] | None = None, *, stdout: TextIO | None = None, stderr: TextIO | None = None) -> int:
    ...
    parser.add_argument("--input", required=True, type=Path, help="Explicit query observation or search miss JSON input.")
    ...
    try:
        ...
        err.write(f"record_search_miss: ERROR: {exc}\n")
        return 2
    ...
    if args.json:
        ...
        out.write("Search Miss report\n")
        out.write(f"status: {report['status']}\n")
        out.write(f"search_miss_id: {summary.get('search_miss_id')}\n")
        out.write(f"search_miss_kind: {summary.get('search_miss_kind')}\n")
