#!/usr/bin/env python3
"""Replay committed H4 code/source/release fixtures offline."""
...
from archive.prototypes.legacy_runtime.connectors.h4_code_source_release.normalizer_common import H4_SOURCE_IDS, build_h4_fixture_replay_result  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--source-id", choices=H4_SOURCE_IDS)
    parser.add_argument("--fixture-root", default="examples/connectors/h4_code_source_release/fixtures")
    ...
    try:
        ...
        summary = {
            "schema_version": "h4_code_source_fixture_replay_summary.v0",
            "status": "pass",
        ...
        if args.json:
            ...
            print("H4 code/source fixture replay", file=stdout)
...
def replay_fixtures(fixture_root: str | Path, source_id: str | None = None) -> list[dict[str, Any]]:
    ...
    source_ids = [source_id] if source_id else list(H4_SOURCE_IDS)
    ...
    for item in source_ids:
        ...
        for fixture_path in sorted(source_dir.glob("*.json")):
            fixture = load_h4_code_source_fixture(fixture_path)
            normalized = normalizer(fixture)
            results.append(build_h4_fixture_replay_result(fixture, normalized))
    return results
...
def _safe_output_dir(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and rel_lower.endswith("/generated"):
            return resolved
        if rel_lower.startswith("examples/connectors/h4_code_source_release/replay_results"):
            return resolved
        raise ValueError(f"refusing output outside approved H4 replay roots: {rel}")
