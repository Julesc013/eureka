#!/usr/bin/env python3
"""Replay committed H8 manuals/docs/standards fixtures offline."""
...
from archive.prototypes.legacy_runtime.connectors.h8_manuals_docs_standards.normalizer_common import H8_SOURCE_IDS, build_h8_fixture_replay_result  # noqa: E402
from scripts.normalize_h8_manuals_docs_fixture import safe_output_path  # noqa: E402

ALLOWED_PREFIXES = ("examples/connectors/h8_manuals_docs_standards/replay_results",)


def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-id", choices=H8_SOURCE_IDS)
    parser.add_argument("--fixture-root", default="examples/connectors/h8_manuals_docs_standards/fixtures")
    ...
    try:
        ...
        summary = {
            "schema_version": "h8_manuals_docs_fixture_replay_summary.v0",
            "status": "pass",
        ...
        if args.json:
            ...
            print("H8 manuals/docs/standards fixture replay", file=stdout)
...
def replay_fixtures(fixture_root: str | Path, source_id: str | None = None) -> list[dict[str, Any]]:
    ...
    for item in ([source_id] if source_id else list(H8_SOURCE_IDS)):
        assert item is not None
        source_dir = root / item
        if not source_dir.is_dir():
            raise ValueError(f"missing fixture directory: {source_dir}")
        module = importlib.import_module(f"archive.prototypes.legacy_runtime.connectors.h8_manuals_docs_standards.{item}")
        for fixture_path in sorted(source_dir.glob("*.json")):
            fixture = load_h8_manuals_docs_fixture(fixture_path)
            normalized = module.normalize(fixture)
            results.append(build_h8_fixture_replay_result(fixture, normalized))
    return results
