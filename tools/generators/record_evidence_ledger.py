#!/usr/bin/env python3
"""Record a fixture-only evidence ledger report from explicit local JSON input."""
...
from runtime.local.foundry import evidence_ledger  # noqa: E402
...
def build_report(input_path: Path, *, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    payload = evidence_ledger.load_json(input_path)
    record = evidence_ledger.build_evidence_ledger_record(payload)
    errors = evidence_ledger.validate_evidence_ledger_record(record)
    summary = evidence_ledger.summarize_evidence_ledger_record(record)
    return {
...
def write_summary(report: Mapping[str, Any], output_path: Path) -> None:
    ...
    text = "\n".join(
        [
            ...
            "",
            f"- Record: {summary.get('evidence_record_id')}",
            f"- Status: {summary.get('evidence_record_status')}",
            f"- Type: {summary.get('evidence_record_type')}",
            f"- Claim type: {summary.get('claim_type')}",
            f"- Claim subject: {summary.get('claim_subject')}",
            f"- Review required: {str(summary.get('review_required')).lower()}",
            f"- Public truth: {str(summary.get('evidence_record_is_public_truth')).lower()}",
            f"- Accepted evidence: {str(summary.get('evidence_record_is_accepted_evidence')).lower()}",
            f"- Master-index mutation: {str(summary.get('evidence_record_can_mutate_master_index')).lower()}",
            "",
        ]
    )
...
def main(argv: Sequence[str] | None = None, *, stdout: TextIO | None = None, stderr: TextIO | None = None) -> int:
    ...
    parser.add_argument("--input", required=True, type=Path, help="Explicit evidence source JSON input.")
    ...
    try:
        ...
        err.write(f"record_evidence_ledger: ERROR: {exc}\n")
        return 2
    ...
    if args.json:
        ...
        summary = report["summary"]
        out.write("Evidence ledger report\n")
        out.write(f"status: {report['status']}\n")
        out.write(f"evidence_record_id: {summary.get('evidence_record_id')}\n")
        out.write(f"evidence_record_type: {summary.get('evidence_record_type')}\n")
        out.write(f"claim_type: {summary.get('claim_type')}\n")
