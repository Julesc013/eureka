#!/usr/bin/env python3
"""Run or preflight H5 vendor/update/driver metadata live probes with fail-closed policy gates."""
...
from archive.prototypes.legacy_runtime.connectors.h5_vendor_update_driver.live_probe_common import (  # noqa: E402
    H5VendorUpdateLiveProbeBlocked,
    SOURCE_CONFIGS,
    build_h5_vendor_update_live_probe_blocked_result,
    build_h5_vendor_update_live_probe_output_bundle,
    build_h5_vendor_update_live_probe_request,
    build_h5_vendor_update_live_probe_result,
    fetch_h5_vendor_update_metadata_once,
    load_h5_vendor_update_live_probe_policy_bundle,
    summarize_h5_vendor_update_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", help="Optional explicit H5 vendor/update live-probe request envelope.")
    ...
    try:
        ...
        if not args.check:
            ...
            if args.vendor_identity_output:
                _write_json(args.vendor_identity_output, result["vendor_identity_candidate"])
            if args.compatibility_output:
                _write_json(args.compatibility_output, result["driver_device_compatibility_candidate"])
            if args.firmware_output:
                _write_json(args.firmware_output, result["firmware_update_candidate"])
            if args.runtime_output:
                _write_json(args.runtime_output, result["runtime_redistributable_candidate"])
            if args.payload_output:
                _write_json(args.payload_output, result["payload_metadata_candidate"])
        ...
        summary = {
            ...
            and any(
                [
                    args.output,
                    args.vendor_identity_output,
                    args.compatibility_output,
                    args.firmware_output,
                    args.runtime_output,
                    args.payload_output,
            ...
            "live_probe": summarize_h5_vendor_update_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H5 vendor/update/driver metadata live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    ...
    if not validation["approved"]:
        result = build_h5_vendor_update_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h5_vendor_update_live_probe_output_bundle(result)}
    if not live:
        result = build_h5_vendor_update_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h5_vendor_update_live_probe_output_bundle(result)}
    try:
        payload, metadata = fetch_h5_vendor_update_metadata_once(request, policy_bundle)
    except H5VendorUpdateLiveProbeBlocked as exc:
        result = exc.result
        return {"live_probe_result": result, "output_bundle": build_h5_vendor_update_live_probe_output_bundle(result)}
    metadata["live_probe_request_ref"] = request.get("live_probe_request_id")
    result = build_h5_vendor_update_live_probe_result(str(request["source_id"]), payload, metadata, policy_bundle)
    return {"live_probe_result": result, "output_bundle": build_h5_vendor_update_live_probe_output_bundle(result)}


def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h5_vendor_update_live_probe_result(result)
    lines = [
        "# H5 Vendor/Update Metadata Live Probe Summary",
        "",
        ...
        f"- vendor_identity_candidate_present: `{str(summary['vendor_identity_candidate_present']).lower()}`",
        f"- driver_device_compatibility_candidate_present: `{str(summary['driver_device_compatibility_candidate_present']).lower()}`",
        f"- firmware_update_candidate_present: `{str(summary['firmware_update_candidate_present']).lower()}`",
        f"- runtime_redistributable_candidate_present: `{str(summary['runtime_redistributable_candidate_present']).lower()}`",
        f"- payload_metadata_candidate_present: `{str(summary['payload_metadata_candidate_present']).lower()}`",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h5_vendor_update_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h5_vendor_update_driver/live_probe_results/"):
            return resolved
