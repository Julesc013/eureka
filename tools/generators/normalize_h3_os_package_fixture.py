#!/usr/bin/env python3
"""Normalize one committed H3 OS package archive fixture offline."""
...
from archive.prototypes.legacy_runtime.connectors.h3_os_package_archives.fixture_loader import load_h3_os_package_fixture  # noqa: E402
from archive.prototypes.legacy_runtime.connectors.h3_os_package_archives.normalizer_common import H3_SOURCE_IDS, summarize_h3_normalized_record  # noqa: E402
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--source-id", required=True, choices=H3_SOURCE_IDS)
    ...
    parser.add_argument("--compatibility-output", help="Optional compatibility candidate output path.")
    ...
    try:
        fixture = load_h3_os_package_fixture(_repo_path(args.input))
        record = _normalizer(args.source_id)(fixture)
        if not args.check:
            if args.output:
                _write_json(args.output, record, {"normalized", "audit_generated", "temp"})
            if args.identity_output:
                _write_json(args.identity_output, record["os_package_identity_candidate"], {"identity", "audit_generated", "temp"})
            if args.compatibility_output:
                _write_json(args.compatibility_output, record["os_platform_compatibility_candidate"], {"identity", "audit_generated", "temp"})
            if args.dependency_output:
                _write_json(args.dependency_output, {"dependency_candidates": record["dependency_candidate_preview"]}, {"identity", "audit_generated", "temp"})
            if args.file_candidate_output:
                _write_json(args.file_candidate_output, {"file_candidates": record["file_candidate_preview"]}, {"identity", "audit_generated", "temp"})
            if args.source_cache_output:
                _write_json(args.source_cache_output, record["source_cache_candidate_preview"], {"audit_generated", "temp"})
            if args.evidence_preview_output:
                _write_json(args.evidence_preview_output, record["evidence_candidate_preview"], {"audit_generated", "temp"})
        if args.json:
            print(json.dumps(record, indent=2, sort_keys=True), file=stdout)
        else:
            summary = summarize_h3_normalized_record(record)
            print("H3 OS package fixture normalizer", file=stdout)
...
def _normalizer(source_id: str):
    module = importlib.import_module(f"archive.prototypes.legacy_runtime.connectors.h3_os_package_archives.{source_id}")
...
def _safe_output_path(path: Path, allowed_kinds: set[str]) -> Path:
    ...
    try:
        ...
        if "audit_generated" in allowed_kinds and rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if "normalized" in allowed_kinds and rel_lower.startswith("examples/connectors/h3_os_package_archives/normalized/"):
            return resolved
        if "identity" in allowed_kinds and rel_lower.startswith("examples/connectors/h3_os_package_archives/identity/"):
            return resolved
