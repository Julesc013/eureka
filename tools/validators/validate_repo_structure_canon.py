...
SCRIPT_WRAPPER_MARKER = "EUREKA_SCRIPT_COMPAT_WRAPPER = True"
...
def _validate_generated_artifact_exceptions(
    ...
    return {
        ...
        "data_public_index_status": str(exact.get("site/dist/data/public_index", {}).get("status", "missing")),
...
def _script_wrapper_report(tracked_files: Sequence[str]) -> dict[str, Any]:
    script_files = sorted(path for path in tracked_files if path.startswith("scripts/") and path.endswith(".py"))
    wrappers: list[str] = []
    ...
    for path in script_files:
        ...
        try:
            ...
            non_wrappers.append(path)
            continue
        if SCRIPT_WRAPPER_MARKER in text:
    ...
    return {
        ...
        "non_wrapper_count": len(non_wrappers),
        "non_wrappers": non_wrappers,
    }
