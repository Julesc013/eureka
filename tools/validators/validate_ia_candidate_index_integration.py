#!/usr/bin/env python3
"""Validate IA-05 Internet Archive candidate-index integration."""
...
from runtime.source.observation.internet_archive_candidate_index import (  # noqa: E402
...
REQUIRED_FILES = (
    ...
    "examples/index/candidate/internet_archive_metadata/expected_candidates.json",
    "examples/index/candidate/internet_archive_metadata/expected_candidate_boundary_report.json",
    "runtime/index/candidate/store.py",
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    _ = argv
    result = validate_ia_candidate_index_integration(REPO_ROOT)
    print(json.dumps(result, indent=2, sort_keys=True), file=stdout)
    return 0 if result["status"] == "pass" else 1


def validate_ia_candidate_index_integration(repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if not _ia04_passed(repo_root):
        errors.append("ia_04_pass_result_not_found")
    for rel_path in REQUIRED_FILES:
        if not (repo_root / rel_path).exists():
            errors.append(f"missing_file:{rel_path}")

    policy = _load_json(repo_root / "control/policies/ia_candidate_index_policy.json", errors)
    _validate_policy(policy, errors)
    evidence_candidates = load_default_ia_evidence_candidates()
    if not evidence_candidates:
        errors.append("evidence_candidates_missing")
    candidates = build_ia_candidates_from_evidence(evidence_candidates[:20], policy) if evidence_candidates else []
    if not candidates:
        errors.append("ia_candidates_missing")
    for candidate in candidates:
        errors.extend(validate_ia_candidate_record(candidate, policy))
    ...
    dry_run = _run(
        [
            ...
            "scripts/eureka_ia_candidate_index_write.py",
            "--instance",
            str(Path(tempfile.gettempdir()) / "eureka-ia05-dry-run"),
            "--operator-token",
            "local-dev-token",
    ...
    if dry_run.returncode != 0:
        errors.append("candidate_dry_run_failed")
    ...
    if apply_result.get("status") != "pass":
        errors.append("temp_instance_candidate_apply_failed")
    ...
    return {
        "schema_version": "ia_candidate_index_integration_validation.v0",
        "task": "IA-05",
        ...
        "temp_instance_candidate_write_passed": apply_result.get("status") == "pass",
        "fixture_candidates_written_to_temp": bool(apply_result.get("fixture_candidates_written_to_temp", False)),
        "live_preview_candidates_written_to_temp": bool(apply_result.get("live_preview_candidates_written_to_temp", False)),
        "all_candidates_require_review": bool(apply_result.get("all_candidates_require_review", False)) or all(
        ...
        "candidate_index_mutated": bool(apply_result.get("candidate_index_mutated", False)),
        "candidate_index_write_scope": "temp_explicit_instance_only" if apply_result.get("status") == "pass" else "none",
...
def _run_temp_apply(repo_root: Path) -> dict[str, Any]:
    warnings: list[str] = []
    with tempfile.TemporaryDirectory(prefix="eureka-ia05-") as tmp:
        ...
        return {
            "status": "pass" if boundary.get("passed") is True and report.get("candidate_index_mutated") is True else "fail",
            "warnings": warnings,
            "fixture_candidates_written_to_temp": bool(report.get("fixture_candidates_written_to_temp", False)),
            "live_preview_candidates_written_to_temp": bool(report.get("live_preview_candidates_written_to_temp", False)),
            "all_candidates_require_review": bool(report.get("all_candidates_require_review", False)),
...
def _validate_policy(policy: Mapping[str, Any], errors: list[str]) -> None:
    if policy.get("schema_version") != "ia_candidate_index_policy.v0":
        errors.append("bad_ia_candidate_policy_schema")
    if policy.get("candidate_index_writes_enabled_for_IA_05") is not True:
        errors.append("candidate_writes_not_enabled_for_ia05")
    ...
    if policy.get("candidates_are_provisional") is not True:
        errors.append("candidates_are_provisional_not_true")
    for key in (
...
def _validate_cli_report(stdout: str, *, expected_write: bool, errors: list[str]) -> None:
    try:
        ...
        errors.append(f"candidate_cli_invalid_json:{exc}")
        return
    report = payload.get("write_report", {})
    boundary = payload.get("boundary_report", {})
    if bool(report.get("candidate_index_mutated", False)) is not expected_write:
        errors.append("candidate_cli_write_flag_mismatch")
    if boundary.get("passed") is not True:
        errors.append("candidate_cli_boundary_failed")
    for key in (
        ...
        if boundary.get(key) is not False:
            errors.append(f"candidate_boundary_forbidden_flag:{key}")


def _ia04_passed(repo_root: Path) -> bool:
    result_path = repo_root / "control/inventory/ia_04_result.json"
    if not result_path.exists():
        return False
    result = json.loads(result_path.read_text(encoding="utf-8"))
    return (
        result.get("status") == "pass"
        and result.get("evidence_ledger_write_performed") is True
