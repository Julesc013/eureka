...
CONTRACTS = tuple("contracts/control_schemas/previews/h11/connectors/" + name for name in ['storefront_live_probe_request.v0.json', 'storefront_live_probe_result.v0.json', 'storefront_live_probe_output_bundle.v0.json', 'storefront_connector_health_summary.v0.json'])
...
PYTHON_FILES = tuple(
    ["archive/prototypes/legacy_runtime/connectors/h11_storefront/live_probe_common.py"]
    + [f"archive/prototypes/legacy_runtime/connectors/h11_storefront/live_probe_{source_id}.py" for source_id in EXPECTED_SOURCES]
    + [
...
def validate_runtime_imports(errors: list[str]) -> None:
    try:
        importlib.import_module("archive.prototypes.legacy_runtime.connectors.h11_storefront.live_probe_common")
        for source_id in EXPECTED_SOURCES:
...
def validate_cli_offline(root: Path, errors: list[str]) -> None:
    ...
    forbidden_checks = [
        ...
        [sys.executable, "scripts/run_h11_storefront_live_probe.py", "--source-id", "fdroid_metadata", "--request-key", "example_app_metadata", "--output", "site/dist/data/public_index/h11.json"],
