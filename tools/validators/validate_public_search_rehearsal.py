...
def _validate_smoke_script(errors: list[str]) -> None:
    if not SMOKE_SCRIPT.is_file():
        ...
        return
    text = _read_script_or_wrapper_target(SMOKE_SCRIPT)
    for token in (
        "CREATED_BY_SLICE = \"public_search_rehearsal_v0\"",
        "SAFE_QUERY_CASES",
...
def _read_script_or_wrapper_target(path: Path) -> str:
    text = path.read_text(encoding="utf-8")
    if "EUREKA_SCRIPT_COMPAT_WRAPPER" not in text:
        return text
    for line in text.splitlines():
        marker = " / '"
        if "_TARGET =" in line and marker in line:
            target = line.split(marker, 1)[1].split("'", 1)[0]
