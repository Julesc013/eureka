"""Validate OBS-AGENT-02 source gap candidate artifacts.

The validator is read-only. It checks that source gap candidates remain
review-gated planning records and do not claim source approval, accepted
evidence, live access, public truth, or product behavior changes.
"""
...
INVENTORY_MANIFEST_PATH = "control/inventory/observations/obs_agent_source_gap_candidate_manifest.json"
AUDIT_REPORT_PATH = "control/audits/obs-agent-02-source-gap-candidate-generation-v0/obs_agent_02_report.json"
AUDIT_MANIFEST_PATH = "control/audits/obs-agent-02-source-gap-candidate-generation-v0/source_gap_candidate_manifest.json"
AUDIT_SUMMARY_PATH = "control/audits/obs-agent-02-source-gap-candidate-generation-v0/source_gap_candidate_summary.md"
AUDIT_QUEUE_PREVIEW_PATH = "control/audits/obs-agent-02-source-gap-candidate-generation-v0/source_policy_decision_queue_preview.md"
DOC_PATH = "docs/operations/OBS_AGENT_SOURCE_GAP_CANDIDATES.md"
...
REVIEW_GATED_STATUSES = {"proposed", "needs_human_review", "policy_blocked", "deferred"}
...
def main(argv: Sequence[str] | None = None, *, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate OBS source gap candidate artifacts.")
    parser.add_argument("--repo-root", default=str(REPO_ROOT), help="Repository root to validate.")
    parser.add_argument("--manifest-file", default=INVENTORY_MANIFEST_PATH, help="Source gap manifest to validate.")
    parser.add_argument("--json", action="store_true", help="Emit deterministic JSON.")
    args = parser.parse_args(list(argv) if argv is not None else None)

    report = validate_source_gap_observation_candidates(Path(args.repo_root), args.manifest_file)
...
def validate_source_gap_observation_candidates(
    ...
    return {
        "schema_version": "obs_agent_source_gap_candidate_validation.v0",
...
def validate_policy_payload(payload: Any, source: str) -> list[str]:
    ...
    errors: list[str] = []
    if data.get("schema_version") != "obs_agent_source_gap_candidate_policy.v0":
        errors.append(f"{source}: schema_version must be obs_agent_source_gap_candidate_policy.v0")
    ...
    errors.extend(_missing_items(_string_items(data.get("allowed_source_access_modes")), ALLOWED_SOURCE_ACCESS_MODES, f"{source}: allowed_source_access_modes"))
    errors.extend(_missing_items(_string_items(data.get("required_review_states")), REVIEW_GATED_STATUSES, f"{source}: required_review_states"))
...
def validate_priority_model_payload(payload: Any, source: str) -> list[str]:
    data = _mapping(payload)
    errors: list[str] = []
    if data.get("schema_version") != "obs_agent_source_gap_priority_model.v0":
        errors.append(f"{source}: schema_version must be obs_agent_source_gap_priority_model.v0")
    score_fields = _mapping(data.get("score_fields"))
    if score_fields.get("minimum") != 0:
        errors.append(f"{source}: score_fields.minimum must be 0")
    if score_fields.get("maximum") != 100:
        errors.append(f"{source}: score_fields.maximum must be 100")
    if not _mapping(data.get("positive_factors")):
        errors.append(f"{source}: positive_factors must be non-empty")
    if not _mapping(data.get("negative_factors")):
        errors.append(f"{source}: negative_factors must be non-empty")
    ...
    for field in ("approves_source_access", "ranks_accepted_truth", "creates_observation", "creates_evidence_truth", "mutates_master_index"):
...
def validate_manifest_payload(payload: Any, source: str, repo_root: Path = REPO_ROOT) -> list[str]:
    data = _mapping(payload)
    errors: list[str] = []
    if data.get("schema_version") != "obs_agent_source_gap_candidate_manifest.v0":
        errors.append(f"{source}: schema_version must be obs_agent_source_gap_candidate_manifest.v0")
    records = [_mapping(item) for item in _sequence_items(data.get("source_gap_candidates"))]
    if data.get("source_gap_candidate_count") != len(records):
        errors.append(f"{source}: source_gap_candidate_count must match source_gap_candidates length")
    ...
    family_counts = Counter(str(record.get("source_family")) for record in records)
    mode_counts = Counter(str(record.get("source_access_mode")) for record in records)
    if data.get("candidate_status_counts") != dict(sorted(status_counts.items())):
        errors.append(f"{source}: candidate_status_counts must match source_gap_candidates")
    if data.get("source_family_counts") != dict(sorted(family_counts.items())):
        errors.append(f"{source}: source_family_counts must match source_gap_candidates")
    if data.get("source_access_mode_counts") != dict(sorted(mode_counts.items())):
        errors.append(f"{source}: source_access_mode_counts must match source_gap_candidates")
...
def validate_manifest_record(record: Mapping[str, Any], source: str) -> list[str]:
    ...
    errors.extend(_source_access_errors(record, f"{source}: {candidate_id}"))
    errors.extend(_priority_score_errors(record.get("priority_score"), f"{source}: {candidate_id}"))
...
def validate_candidate_payload(payload: Any, source: str) -> list[str]:
    ...
    for field in ("source_family", "source_gap_summary", "why_this_source_family", "recommended_next_review_action", "priority_score"):
        if field not in data:
            errors.append(f"{source}: missing source-gap field {field}")
    errors.extend(_source_access_errors(data, source))
    errors.extend(_priority_score_errors(data.get("priority_score"), source))
    errors.extend(_boundary_false_errors(_mapping(data.get("product_boundary")), source, include_extra=False))
    if _mapping(data.get("product_boundary")).get("approved_source_access") is not False:
        errors.append(f"{source}: product_boundary.approved_source_access must be false")
    if _mapping(data.get("product_boundary")).get("modified_track_b_files") is not False:
        errors.append(f"{source}: product_boundary.modified_track_b_files must be false")
    errors.extend(_forbidden_text_errors(data, source))
...
def validate_audit_report_payload(payload: Any, source: str) -> list[str]:
    data = _mapping(payload)
    errors: list[str] = []
    if data.get("schema_version") != "obs_agent_02_report.v0":
        errors.append(f"{source}: schema_version must be obs_agent_02_report.v0")
    if data.get("track") != "Observation":
        errors.append(f"{source}: track must be Observation")
    if data.get("task") != "OBS-AGENT-02":
        errors.append(f"{source}: task must be OBS-AGENT-02")
...
def _validate_docs(repo_root: Path) -> list[str]:
    ...
    for path in REQUIRED_DOCS:
        ...
        if path == DOC_PATH:
            for phrase in ("review-gated", "repo-local", "source approval", "track b"):
                if phrase not in text:
                    errors.append(f"{path}: missing phrase {phrase!r}")
    for path in (AUDIT_SUMMARY_PATH, AUDIT_QUEUE_PREVIEW_PATH):
...
def _source_access_errors(record: Mapping[str, Any], source: str) -> list[str]:
    errors: list[str] = []
    ...
    if mode not in ALLOWED_SOURCE_ACCESS_MODES:
        errors.append(f"{source}: source_access_mode {mode!r} is not allowed")
    if mode in {"approved_api_future", "approved_metadata_probe_future", "approved_static_dump_future"}:
        if not any(marker in policy_status for marker in ("future", "deferred", "required")):
            errors.append(f"{source}: future source mode must remain future/deferred or policy-required")
    if record.get("candidate_status") == "policy_blocked" and "block" not in policy_status:
        errors.append(f"{source}: policy_blocked candidate must remain blocked")
    if record.get("candidate_status") not in REVIEW_GATED_STATUSES:
        errors.append(f"{source}: candidate_status must remain review-gated")
    return errors


def _priority_score_errors(score: Any, source: str) -> list[str]:
    if not isinstance(score, int) or score < 0 or score > 100:
        return [f"{source}: priority_score must be an integer from 0 to 100"]
    return []


def _priority_summary_errors(summary: Mapping[str, Any], records: Sequence[Mapping[str, Any]], source: str) -> list[str]:
    scores = [record.get("priority_score") for record in records if isinstance(record.get("priority_score"), int)]
    expected = {"min": 0, "max": 0, "average": 0} if not scores else {
        "min": min(scores),
        "max": max(scores),
        "average": round(sum(scores) / len(scores), 2),
    }
    return [f"{source}: priority_score_summary must match candidate priority scores"] if summary != expected else []
...
def _format_plain(report: Mapping[str, Any]) -> str:
    lines = [
        f"validate_source_gap_observation_candidates: {report['status']}",
