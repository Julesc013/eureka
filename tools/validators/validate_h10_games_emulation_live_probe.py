...
CONTRACTS = tuple(['contracts/control_schemas/previews/h10/connectors/games_emulation_live_probe_request.v0.json', 'contracts/control_schemas/previews/h10/connectors/games_emulation_live_probe_result.v0.json', 'contracts/control_schemas/previews/h10/connectors/games_emulation_live_probe_output_bundle.v0.json', 'contracts/control_schemas/previews/h10/connectors/games_emulation_connector_health_summary.v0.json'])
...
PYTHON_FILES = tuple(
    ["archive/prototypes/legacy_runtime/connectors/h10_games_emulation/live_probe_common.py"]
    + [f"archive/prototypes/legacy_runtime/connectors/h10_games_emulation/live_probe_{source_id}.py" for source_id in EXPECTED_SOURCES]
    + [
...
def validate_runtime_imports(errors: list[str]) -> None:
    try:
        importlib.import_module("archive.prototypes.legacy_runtime.connectors.h10_games_emulation.live_probe_common")
        for source_id in EXPECTED_SOURCES:
            importlib.import_module(f"archive.prototypes.legacy_runtime.connectors.h10_games_emulation.live_probe_{source_id}")
...
def validate_cli_offline(root: Path, errors: list[str]) -> None:
    ...
    for forbidden_path in ("site/dist/data/public_index/probe.json", "roms/probe.json", "bios/probe.json", "emulators/probe.json", "actions/probe.json"):
        run = subprocess.run(
