...
CONTRACTS = tuple("contracts/control_schemas/previews/h12/connectors/" + name for name in ('retro_community_live_probe_request.v0.json', 'retro_community_live_probe_result.v0.json', 'retro_community_live_probe_output_bundle.v0.json', 'retro_community_connector_health_summary.v0.json'))
...
PYTHON_FILES = tuple(
    ["archive/prototypes/legacy_runtime/connectors/h12_retro_community/live_probe_common.py"]
    + [f"archive/prototypes/legacy_runtime/connectors/h12_retro_community/live_probe_{source_id}.py" for source_id in EXPECTED_SOURCES]
    + [
...
def validate_runtime_imports(errors: list[str]) -> None:
    try:
        importlib.import_module("archive.prototypes.legacy_runtime.connectors.h12_retro_community.live_probe_common")
...
def validate_cli_offline(root: Path, errors: list[str]) -> None:
    ...
    forbidden_checks = [
        ...
        [sys.executable, "scripts/run_h12_retro_community_live_probe.py", "--source-id", "winworld_metadata", "--request-key", "example_catalog_item_metadata", "--output", "site/dist/data/public_index/h12.json"],
