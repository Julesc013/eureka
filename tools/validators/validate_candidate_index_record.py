...
EXAMPLES_ROOT = REPO_ROOT / "examples" / "index" / "candidate"
...
def validate_all_examples(*, strict: bool = False) -> dict[str, Any]:
    ...
    if not EXAMPLES_ROOT.is_dir():
        errors.append("examples/index/candidate: missing examples root.")
    else:
        roots = sorted(
            path
        ...
        if not roots:
            errors.append("examples/index/candidate: no example roots found.")
        for root in roots:
