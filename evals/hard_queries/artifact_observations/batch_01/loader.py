"""Loader, validation, and projection helpers for artifact observation batch one."""
...
EXPECTED_REVIEWABLE_COUNT = 6
EXPECTED_SOURCE_REFERENCE_COUNT = 12
...
def validate_artifact_observations(payload: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if len(records) != EXPECTED_OBSERVATION_COUNT:
        errors.append(f"batch must contain {EXPECTED_OBSERVATION_COUNT} manual artifact observations")
...
def validate_reviewable_artifact_items(payload: Mapping[str, Any], observations: Mapping[str, Any] | None = None) -> tuple[str, ...]:
    ...
    if len(records) != EXPECTED_REVIEWABLE_COUNT:
        errors.append(f"batch must contain {EXPECTED_REVIEWABLE_COUNT} reviewable artifact items")
    observation_ids = {item["observation_id"] for item in observation_records(observations or load_artifact_observations())}
...
def validate_evidence_level_summary(payload: Mapping[str, Any], observations: Mapping[str, Any] | None = None) -> tuple[str, ...]:
    ...
    for key, value in expected_levels.items():
        if int((payload.get("artifact_level_counts") or {}).get(key, -1)) != value:
            errors.append(f"{key} count mismatch")
    for key, value in expected_statuses.items():
        if int((payload.get("status_counts") or {}).get(key, -1)) != value:
            errors.append(f"{key} status count mismatch")
    if int(payload.get("manual_artifact_observation_count", -1)) != EXPECTED_OBSERVATION_COUNT:
        errors.append("manual_artifact_observation_count mismatch")
    if int(payload.get("reviewable_artifact_item_count", -1)) != EXPECTED_REVIEWABLE_COUNT:
        errors.append("reviewable_artifact_item_count mismatch")
...
def validate_public_alpha_artifact_gate(payload: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    expected = {
        ...
        "new_verified_artifact_count": 0,
        "manual_artifact_observation_count": EXPECTED_OBSERVATION_COUNT,
        "reviewable_artifact_item_count": EXPECTED_REVIEWABLE_COUNT,
    }
    ...
    if payload.get("next_recommended_task") != "HUMAN-ARTIFACT-REVIEW-BATCH-01":
        errors.append("next task must be HUMAN-ARTIFACT-REVIEW-BATCH-01")
    return tuple(errors)
...
def validate_source_reference_index(payload: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if len(refs) != EXPECTED_SOURCE_REFERENCE_COUNT:
        errors.append(f"source reference index must contain {EXPECTED_SOURCE_REFERENCE_COUNT} manual references")
    for item in refs:
...
def project_artifact_observation(observation: Mapping[str, Any], profile: str, *, visibility_posture: str = "public") -> dict[str, Any]:
    return SurfaceKernel().project(
        SurfaceRequest(
            ...
            data_version="manual-artifact-observation-batch-01",
        )
    )
...
def _resolution_run_for_observation(observation: Mapping[str, Any]) -> ResolutionRunRecord:
    ...
    return ResolutionRunRecord(
        ...
        run_kind="manual_artifact_observation_batch_01_projection",
...
def _fallback_summary(observation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    status = _projected_status(observation)
    summary = {
        "schema_version": "manual_artifact_observation_batch_01_fallback_summary.v0",
        "mode": "manual_artifact_observation_batch_01",
        "status": status,
        ...
        "source_observation_refs": [f"manual_artifact_observation_batch_01:{observation_id}"],
        "evidence_refs": [f"manual_artifact_observation_batch_01:{observation_id}"],
