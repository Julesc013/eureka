# H14 Source Discovery Rollup Approval Gates

A dry-run request is accepted only when committed policy says `rollup_dry_run_approved: true`, the request key is allowlisted, `operation_scope` is `rollup_dry_run_only`, the operation class is `rollup_preview_only`, all live/access/model/mutation/pack/write/truth approval flags are false, output paths are allowlisted, and the kill switch is fail-closed.
