...
python scripts/eureka_search_hunt.py --instance ./eureka-instance list --json
python scripts/eureka_search_hunt.py --instance ./eureka-instance show --id <session-id> --with-transitions --with-summaries --json
```

Record a basic state transition:

```powershell
python scripts/eureka_search_hunt.py --instance ./eureka-instance transition --id <session-id> --state running --reason "operator inspection" --json
```

Run the demo and validator:

```powershell
python scripts/demo_search_hunt_session.py --instance ./eureka-instance --query "sampleproject" --json
