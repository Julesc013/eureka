...
```text
python scripts/check_full_discovery.py --run-id source_snapshot_full_discovery_rerun_07 --json
```
...
Commands for this ingest:

```text
python -m json.tool docs\reference\validation\source_snapshot_full_discovery_ingest_07\FULL_DISCOVERY_SUMMARY_INDEX.json
python -m json.tool docs\reference\validation\source_snapshot_full_discovery_ingest_07\FAILURE_FAMILY_INDEX.json
git diff --check
...
ia_metadata_provider_wiring: deferred_by_queue_authority
```
