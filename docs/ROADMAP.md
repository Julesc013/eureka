...
v0, Live Probe Gateway Contract v0, Rust Query Planner Parity Candidate v0, and Compatibility Surface Strategy v0 now
mark the start of a more evidence-led backend phase. The next step is to define
the signed/offline snapshot format without adding live behavior.
Rust Query Planner Parity Candidate v0 remains isolated and does not wire Rust
into runtime behavior. Manual
...
work that requires explicit human approval after this contract and separate
