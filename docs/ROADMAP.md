...
Resolution Run Model v0 now also lives under `runtime/engine/resolution_runs/`, giving the repo a first synchronous durable investigation envelope without introducing worker queues, streaming updates, or query-planner semantics.
...
Source Registry v0 and Resolution Run Model v0 now mark the start of the
operational backend phase. The next step is to introduce planner-owned
structured intent without prematurely jumping to workers, public hosting,
