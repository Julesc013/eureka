...
7. Persist the observation flow to `runtime/source_cache`.
8. Persist an evidence candidate to `runtime/evidence_ledger`.
9. Enqueue a review item in `runtime/review_queue`.
10. Record an explicit local review decision.
11. Rebuild `runtime/public_index` from accepted local review records.
