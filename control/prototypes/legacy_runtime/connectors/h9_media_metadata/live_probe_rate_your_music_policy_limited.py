"""Source-specific fail-closed H9 live-probe wrapper for Rate Your Music metadata, policy-limited."""
...
from control.prototypes.legacy_runtime.connectors.h9_media_metadata.live_probe_common import (
    SOURCE_CONFIGS,
    build_h9_media_metadata_live_probe_result,
)

SOURCE_ID = "rate_your_music_policy_limited"
...
def build_request_url_or_metadata_request(request: Mapping[str, Any], policy_bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if request.get("source_id") != SOURCE_ID:
        raise ValueError("request source_id does not match rate_your_music_policy_limited")
