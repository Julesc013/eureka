"""Offline H3 OS package archive quality delta helpers."""
...
from control.prototypes.legacy_runtime.connectors.h3_os_package_archives.normalizer_common import H3_SOURCE_IDS
from control.prototypes.legacy_runtime.connectors.h3_os_package_archives.review_integration import (
...
def build_h3_quality_delta(inputs: Mapping[str, Any], policy: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """Build a bounded H3 quality delta from review integration outputs."""
    ...
    metrics = {
        "source_count": len(sources) or len(H3_SOURCE_IDS),
        ...
        "os_package_identity_candidate_count": len(review.get("os_package_identity_review_seeds", [])),
        "os_platform_compatibility_candidate_count": len(review.get("os_platform_compatibility_review_seeds", [])),
        "dependency_candidate_count": len(review.get("dependency_candidate_review_seeds", [])),
        "conflict_candidate_count": len(review.get("conflict_candidate_review_seeds", [])),
        "provides_candidate_count": len(review.get("provides_candidate_review_seeds", [])),
        ...
        "review_seed_count": sum(
            len(review.get(key, []))
            for key in (
                "os_package_identity_review_seeds",
                "os_platform_compatibility_review_seeds",
                "dependency_candidate_review_seeds",
                "conflict_candidate_review_seeds",
                "provides_candidate_review_seeds",
    ...
    per_source = [
        {
            ...
            "compatibility_review_seed_created": source_id in sources,
        ...
        for source_id in sorted(set(sources) or set(H3_SOURCE_IDS))
    ]
    delta = {
        "schema_version": "h3_os_package_quality_delta_report.v0",
        "quality_delta_id": f"h3.quality_delta.{_digest(review)[:12]}.v0",
        "wave_id": "H3",
        "comparison_scope": "fixture_review_and_blocked_live_probe_evidence",
        **metrics,
        "per_source_deltas": per_source,
        "limitations": [
            "Quality delta measures H3 review readiness only.",
            "Blocked live probes do not prove endpoint behavior.",
            "OS package archive metadata is not installability, compatibility, safety, rights, or dependency correctness proof.",
        ],
        "forbidden_claims": [
            "production_search_quality",
            "production_archive_coverage",
        ...
        "notes": ["H3 quality delta is operational evidence only."],
    }
    errors = detect_h3_quality_overclaim(delta, policy)
    if errors:
        raise ValueError("; ".join(errors))
    return delta


def summarize_h3_quality_delta(delta: Mapping[str, Any], policy: Mapping[str, Any] | None = None) -> dict[str, Any]:
    errors = detect_h3_quality_overclaim(delta, policy)
    return {
        "schema_version": "h3_quality_delta_summary.v0",
        ...
        "os_package_identity_candidate_count": delta.get("os_package_identity_candidate_count", 0),
        "os_platform_compatibility_candidate_count": delta.get("os_platform_compatibility_candidate_count", 0),
        "dependency_candidate_count": delta.get("dependency_candidate_count", 0),
        "conflict_candidate_count": delta.get("conflict_candidate_count", 0),
        "provides_candidate_count": delta.get("provides_candidate_count", 0),
...
def detect_h3_quality_overclaim(delta: Mapping[str, Any], policy: Mapping[str, Any] | None = None) -> list[str]:
    ...
    errors.extend(detect_h3_review_truth_boundary_violations(delta, policy))
    errors.extend(detect_h3_review_product_boundary_violations(delta, policy))
...
def _known_gaps(review: Mapping[str, Any]) -> list[str]:
    ...
    if len(review.get("source_cache_review_seeds", [])) < len(H3_SOURCE_IDS):
...
def _truth_boundary() -> dict[str, bool]:
    return {
        ...
        "production_archive_coverage": False,
        ...
        "repository_index_coverage_complete": False,
