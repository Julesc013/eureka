...
- `git status --short` - PASS before work and PASS after commit.
- `git diff --check` - PASS.
...
- `python scripts/check_git_task_state.py --mode start-task --task-id SYNC-GUARD-01 --allow-main` - PASS after the task branch is pushed with upstream.
- `python scripts/check_git_task_state.py --mode start-task --task-id SYNC-GUARD-01 --allow-main --json` - PASS after the task branch is pushed with upstream.
