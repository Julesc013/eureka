# Public Alpha Reassess Matrix

- accepted_truth: false
- accepted_truth_created: false
- batch_id: seed_batch_driver_support_00
- candidate_count: 16
- compatibility_guarantee_created: false
- cracks_keygens_serials_supported: false
- created_at: 2026-06-02T00:00:00Z
- deployment_performed: false
- domain_id: driver_support_media
- download_performed: false
- driver_updater_spam_supported: false
- extraction_executed: false
- file_fetch_performed: false
- install_execution_enabled: false
- malware_clean_claim_created: false
- master_index_mutated: false
- model_provider_used: false
- operator_instance_mutated: false
- production_readiness_claimed: false
- public_alpha_reassess_id: public_alpha_reassess_input:f6e4ed9764884983
- public_index_mutated: false
- public_launch_readiness_claimed: false
- public_mutation_enabled: false
- query_count: 16
- raw_live_response_committed: false
- reassess_note: Use after review/local-apply/snapshot gates; this seed batch adds high-risk driver/support discovery but is not launch readiness.
- review_required: true
- reviewed_index_mutated: false
- rights_clearance_claim_created: false
- schema_version: seed_batch_public_alpha_reassess_input.v0
