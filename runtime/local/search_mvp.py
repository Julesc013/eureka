...
TASK_ID = "LOCAL-METADATA-FALLBACK-E2E-DEMO-00"
DATA_VERSION = "local-metadata-fallback-demo-v0"
REPO_ROOT = Path(__file__).resolve().parents[2]
DEMO_FIXTURE_PATH = REPO_ROOT / "evals" / "hard_queries" / "local_metadata_fallback_demo" / "ia_metadata_fixtures.json"
SUPPORTED_METADATA_FALLBACKS = ("none", "ia_fixture")
CANONICAL_STATUSES = (
    "verified",
    "candidate",
    "need",
...
class LocalSearchService:
    ...
    def search(self, query: str, options: LocalSearchOptions | None = None) -> dict[str, Any]:
        ...
        if opts.metadata_fallback == "ia_fixture" and ia_fixture_case is not None:
    ...
    def search_many(self, queries: Sequence[str], options: LocalSearchOptions | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "provider_family": "ia_fixture" if opts.metadata_fallback == "ia_fixture" else "none",
            "limit": opts.limit,
    ...
    def _run_resolution_search(
        ...
        provider: IAFixtureMetadataProvider | None,
    ) -> ResolutionRunRecord:
...
def render_search_html(response: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "</form>",
            '<p class="meta">Local fallback demo. Read-only. Fixture metadata fallback is candidate/need/near-miss only and never verified truth.</p>',
            "</header>",
            "<main>",
            sections,
            "</main>",
...
def status_payload(metadata_fallback: str = "none") -> dict[str, Any]:
    ...
    return {
        ...
        "fallback_mode": "ia_fixture_metadata_fallback" if fallback == "ia_fixture" else "none",
        "provider_family": "ia_fixture" if fallback == "ia_fixture" else "none",
...
def _response_from_projection(
    *,
    ...
    fallback_used = bool(fallback and source_path == "ia_fixture_metadata_fallback")
...
def _card_from_item(
    ...
    *,
    ...
    return {
        ...
        "non_verified_reason": "fixture-derived result requires review and is not accepted truth",
        "verified": False,
        "accepted_truth": False,
...
def _build_resolution_service(
    root: str,
    fallback_provider: IAFixtureMetadataProvider | None,
...
def _truth_boundary_flags() -> dict[str, bool | int]:
    return {
...
def _no_mutation_indicator() -> dict[str, bool | str]:
    return {
        ...
        "note": "Local search and fixture metadata fallback are read-only.",
    }
