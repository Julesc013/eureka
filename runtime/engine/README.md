...
- bounded local indexing for the current mixed catalog under `index/`, building a local SQLite search substrate with FTS5 preferred and deterministic fallback query behavior, preserving compact source ids, source families, resolved-resource identity, representation summaries, synthetic member records, parent lineage, member paths, evidence summaries, source-registry records, and bounded lane/user-cost annotations without implying production ranking, fuzzy retrieval, vector search, live sync, or incremental indexing
