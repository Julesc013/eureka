...
from runtime.review.queue import ReviewDecision, ReviewDecisionKind, ReviewItemRecord, ReviewQueueStatus
...
from runtime.source.observation.internet_archive_metadata import SOURCE_ID
...
def _review_item(candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "item_identifier": str(candidate.get("item_identifier", "")),
        "mediatype": str(candidate.get("mediatype", "")),
        "collection_refs": list(candidate.get("collection_refs", []) or []),
        "file_summary": dict(candidate.get("file_summary", {}) or {}),
        "checksum_summary": dict(candidate.get("checksum_summary", {}) or {}),
        "claim_summary": dict(candidate.get("claim_summary", {}) or {}),
...
def _candidate_snapshot(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "item_identifier": str(item.get("item_identifier", "")),
        "mediatype": str(item.get("mediatype", "")),
        "collection_refs": list(item.get("collection_refs", []) or []),
        "file_summary": dict(item.get("file_summary", {}) or {}),
        "checksum_summary": dict(item.get("checksum_summary", {}) or {}),
        "claim_summary": dict(item.get("claim_summary", {}) or {}),
...
def _review_queue_decision_payload(decision: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_detail": {
            "item_identifier": str(snapshot.get("item_identifier", "")),
            "mediatype": str(snapshot.get("mediatype", "")),
            "collection_refs": list(snapshot.get("collection_refs", []) or []),
            "file_summary": dict(snapshot.get("file_summary", {}) or {}),
            "checksum_summary": dict(snapshot.get("checksum_summary", {}) or {}),
            "claim_summary": dict(snapshot.get("claim_summary", {}) or {}),
        },
        "acceptance_state": "not_accepted",
        "downstream_effects": {
            "reviewed_index": "none",
