...
def member_access_envelope_to_view_model(envelope: Mapping[str, Any]) -> dict[str, Any]:
    view_model: dict[str, Any] = {
        "status": _require_string(envelope.get("status"), "member_access.status"),
        "member_access_status": _require_string(
            envelope.get("member_access_status"),
            "member_access.member_access_status",
        ),
        "target_ref": _require_string(envelope.get("target_ref"), "member_access.target_ref"),
        "representation_id": _require_string(
            envelope.get("representation_id"),
            "member_access.representation_id",
        ),
        "member_path": _require_string(envelope.get("member_path"), "member_access.member_path"),
        ...
        "notices": _coerce_notice_list(envelope.get("notices"), "member_access.notices"),
    }
    for field_name in (
        ...
        optional = _optional_string(envelope.get(field_name), f"member_access.{field_name}")
        if optional is not None:
            view_model[field_name] = optional
    byte_length = envelope.get("byte_length")
    if byte_length is not None:
        view_model["byte_length"] = _require_int(byte_length, "member_access.byte_length")
    return view_model
