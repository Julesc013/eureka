from __future__ import annotations

from pathlib import Path

from runtime.connectors.internet_archive_recorded.source import (
    load_internet_archive_recorded_source_records,
)
from runtime.engine.interfaces.ingest import InternetArchiveRecordedSourceRecord


class InternetArchiveRecordedConnector:
    def __init__(self, fixture_path: Path | None = None) -> None:
        self._fixture_path = fixture_path

    def load_source_records(self) -> tuple[InternetArchiveRecordedSourceRecord, ...]:
        return load_internet_archive_recorded_source_records(self._fixture_path)

    def default_target_ref(self) -> str:
        records = self.load_source_records()
        if not records:
            raise ValueError("Internet Archive recorded connector returned no source records.")
        return records[0].target_ref
