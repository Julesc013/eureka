...
RUNNER = REPO_ROOT / "scripts/run_h1_metadata_live_probe.py"
SUMMARY = REPO_ROOT / "scripts/summarize_h1_live_probe_outputs.py"
VALIDATOR = REPO_ROOT / "scripts/validate_h1_metadata_live_probe.py"
GENERATED = REPO_ROOT / "control/audits/h1-bundle-03-metadata-live-probes-v0/generated"
...
class H1MetadataLiveProbeScriptTests(unittest.TestCase):
    def test_cli_writes_no_files_by_default(self):
        before = sorted(path.name for path in GENERATED.glob("*"))
        result = run_cmd(["scripts/run_h1_metadata_live_probe.py", "--source-id", "pypi", "--request-key", "example_project_metadata"])
    ...
    def test_cli_live_without_approval_is_blocked_offline(self):
        result = run_cmd(["scripts/run_h1_metadata_live_probe.py", "--source-id", "pypi", "--request-key", "example_project_metadata", "--live", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_cli_writes_explicit_outputs_to_temp_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertEqual(json.loads((tmp_path / "result.json").read_text(encoding="utf-8"))["source_id"], "pypi")
            self.assertTrue((tmp_path / "summary.md").is_file())

    def test_cli_refuses_site_dist_output(self):
        result = run_cmd(["scripts/run_h1_metadata_live_probe.py", "--source-id", "pypi", "--request-key", "example_project_metadata", "--output", "site/dist/h1.json"])
    ...
    def test_summary_script_passes_on_examples(self):
        result = run_cmd(["scripts/summarize_h1_live_probe_outputs.py", "--input", "examples/connectors/h1_metadata_wave/live_probe_results", "--check", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        payload = json.loads(result.stdout)
        self.assertFalse(payload["network_used"])
        self.assertGreaterEqual(payload["blocked_count"], 7)

    def test_validator_passes_current_repo(self):
        result = run_cmd(["scripts/validate_h1_metadata_live_probe.py", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_validator_does_not_create_local_private_roots(self):
        result = run_cmd(["scripts/validate_h1_metadata_live_probe.py"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
