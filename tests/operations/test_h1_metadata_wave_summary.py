...
SUMMARY = REPO_ROOT / "scripts/summarize_h1_metadata_wave_sources.py"
VALIDATOR = REPO_ROOT / "scripts/validate_h1_metadata_wave_policy_packs.py"
GENERATED_DIR = REPO_ROOT / "control/audits/h1-bundle-01-metadata-wave-policy-packs-v0/generated"
...
class H1MetadataWaveSummaryTests(unittest.TestCase):
    def test_summary_script_writes_no_files_by_default(self):
        before = sorted(path.name for path in GENERATED_DIR.glob("*"))
        result = run_cmd(["scripts/summarize_h1_metadata_wave_sources.py", "--json"])
    ...
    def test_summary_script_writes_explicit_outputs_to_temp_paths(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            output = tmp_path / "h1-summary.json"
            markdown = tmp_path / "h1-summary.md"
            result = run_cmd([
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertEqual(json.loads(output.read_text(encoding="utf-8"))["source_count"], 7)
            self.assertTrue(markdown.read_text(encoding="utf-8").startswith("# H1 Metadata Wave Source Summary"))

    def test_summary_script_refuses_site_dist_output(self):
        result = run_cmd(["scripts/summarize_h1_metadata_wave_sources.py", "--output", "site/dist/h1-summary.json"])
    ...
    def test_validator_default_mode_is_offline_and_passes(self):
        result = run_cmd(["scripts/validate_h1_metadata_wave_policy_packs.py", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        payload = json.loads(result.stdout)
    ...
    def test_validator_does_not_create_local_private_roots(self):
        result = run_cmd(["scripts/validate_h1_metadata_wave_policy_packs.py"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
