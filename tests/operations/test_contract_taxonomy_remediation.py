...
class ContractTaxonomyRemediationTests(unittest.TestCase):
    ...
    def test_resolver_classifies_archive_fixture_contracts_as_fixture_schemas(self) -> None:
        ...
        self.assertTrue(spec["target_path"].startswith("control/schemas/fixtures/archive/"))
    ...
    def test_resolver_classifies_h14_candidates_as_control_previews(self) -> None:
        ...
        self.assertTrue(all(spec["target_path"].startswith("control/schemas/previews/h14/") for spec in h14))

    def test_resolver_moves_node_and_query_non_product_schemas(self) -> None:
        self.assertEqual(resolver.REMEDIATION_MOVES["contracts/node/work_unit.v0.json"]["target_path"], "control/schemas/policies/node/work_unit.v0.json")
        self.assertEqual(
            resolver.REMEDIATION_MOVES["contracts/query/observation_candidate_review_queue.v0.json"]["target_path"],
            "control/schemas/tasks/query/observation_candidate_review_queue.v0.json",
        )
    ...
    def test_resolver_updates_active_references_and_retires_shims(self) -> None:
        with self.make_repo() as temp, self.taxonomy_patch():
            ...
            self.assertIn(
                "control/schemas/previews/h14/connectors/source_need_candidate.v0.json",
    ...
    def test_validator_fails_if_fixture_schema_remains_under_contracts(self) -> None:
        with self.make_repo() as temp:
            ...
            write_json(
                ...
                {"schema_version": "r0_contract_taxonomy_resolved_items.v0", "resolved": [{"source_path": "contracts/archive/fixtures/software/synthetic_resolution_fixture.json", "target_path": "control/schemas/fixtures/archive/software/synthetic_resolution_fixture.json", "classification": "fixture_schema"}]},
            )
            errors: list[str] = []
