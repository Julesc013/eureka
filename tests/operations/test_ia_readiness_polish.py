...
class IAReadinessPolishTest(unittest.TestCase):
    ...
    def test_latest_task_packet_h1_progression_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.make_fixture(root)
            write_text(
                ...
                "\n".join(
                    [
                        ...
                        "## GOAL",
                        "Main development lane proceeds to H1-BUNDLE-03 after H0 exit; HUMAN-OBS-REVIEW-01 is a parallel side-lane.",
                    ]
                ),
            )
            result = validator.validate_repo(root)
            ...
            write_text(
                ...
                "\n".join(
                    [
                        ...
                        "## GOAL",
                        "Main development lane proceeds to H2-BUNDLE-03 after H2 fixture runtime; HUMAN-OBS-REVIEW-01 is a parallel side-lane.",
                    ]
                ),
            )
            result = validator.validate_repo(root)
    ...
    def test_latest_task_packet_h3_progression_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.make_fixture(root)
            write_text(
                ...
                "\n".join(
                    [
                        ...
                        "## GOAL",
                        "Main development lane proceeds to H3-BUNDLE-02 after H3 policy packs; HUMAN-OBS-REVIEW-01 is a parallel side-lane.",
                    ]
                ),
            )
            result = validator.validate_repo(root)
    ...
    def test_latest_task_packet_h9_progression_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.make_fixture(root)
            write_text(
                ...
                "\n".join(
                    [
                        "# AIDE Latest Task Packet",
                        "## PHASE",
                        "H9-BUNDLE-02 - Media metadata fixture runtimes and normalizers",
                        "## GOAL",
                        "Main development lane proceeds to H9-BUNDLE-02 after H9 policy packs; HUMAN-OBS-REVIEW-01 is a parallel side-lane.",
                    ]
                ),
            )
            result = validator.validate_repo(root)
    ...
    def test_latest_task_packet_public_alpha_local_progression_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.make_fixture(root)
            write_text(
                ...
                "\n".join(
                    [
                        ...
                        "## GOAL",
                        "PUBLIC-ALPHA-DEPLOYMENT-PLAN-01 completed planning only; HUMAN-OBS-REVIEW-01 is a parallel side-lane.",
                    ]
                ),
            )
            result = validator.validate_repo(root)
