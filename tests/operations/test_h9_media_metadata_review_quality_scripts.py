"""CLI tests for H9 media metadata review-quality scripts."""
...
class H9ReviewQualityScriptTests(unittest.TestCase):
    ...
    def test_integrate_writes_no_files_by_default_and_check_passes(self):
        proc = self.run_script("scripts/integrate_h9_media_metadata_review.py", "--input-dir", "examples/connectors/h9_media_metadata/replay_results", "--check")
        self.assertEqual(0, proc.returncode, proc.stdout + proc.stderr)
        self.assertIn("wrote_files: false", proc.stdout)

    def test_integrate_writes_explicit_temp_output(self):
        with tempfile.TemporaryDirectory() as tmp:
            proc = self.run_script("scripts/integrate_h9_media_metadata_review.py", "--input-dir", "examples/connectors/h9_media_metadata/replay_results", "--output-dir", tmp)
            self.assertEqual(0, proc.returncode, proc.stdout + proc.stderr)
            self.assertTrue((Path(tmp) / "h9_review_integration_result_v0.json").is_file())

    def test_quality_delta_writes_explicit_temp_output(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "delta.json"
            proc = self.run_script("scripts/summarize_h9_media_metadata_quality_delta.py", "--input-dir", "examples/connectors/h9_media_metadata/review_integration", "--output", str(out))
            self.assertEqual(0, proc.returncode, proc.stdout + proc.stderr)
    ...
    def test_validator_passes_current_repo(self):
        proc = self.run_script("scripts/validate_h9_media_metadata_review_quality_audit.py")
        self.assertEqual(0, proc.returncode, proc.stdout + proc.stderr)
