...
class LocalRuntimeCompositionScriptTests(unittest.TestCase):
    def test_status_script_prints_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(9, data["store_count"])
            self.assertIn("search_hunt", data["stores"])
            self.assertIn("search_need", data["stores"])
            self.assertIn("agent_research", data["stores"])
            self.assertIn("ai_escalation", data["stores"])
