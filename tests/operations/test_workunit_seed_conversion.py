...
from scripts.build_workunit_seed_candidates import main as build_main
from scripts.summarize_workunit_seed_candidates import main as summarize_main
from scripts.validate_workunit_seed_candidates import (
    ...
    validate_workunit_seed_candidates,
)
...
class WorkUnitSeedConversionTest(unittest.TestCase):
    def test_conversion_policy_validates(self) -> None:
        policy = _read_json(REPO_ROOT / "control/inventory/observations/workunit_seed_conversion_policy.json")

        self.assertEqual(validate_policy_payload(policy, "policy"), [])

    def test_priority_model_validates(self) -> None:
        model = _read_json(REPO_ROOT / "control/inventory/observations/workunit_seed_priority_model.json")
    ...
    def test_build_script_list_inputs_output_is_non_empty(self) -> None:
        ...
        self.assertIn("contracts/query/workunit_seed.v0.json", lines)
    ...
    def test_validator_passes_current_repo(self) -> None:
        report = validate_workunit_seed_candidates(REPO_ROOT)

        self.assertEqual(report["status"], "valid")
        self.assertEqual(report["errors"], [])

    def test_seed_with_execution_allowed_now_true_fails(self) -> None:
        seed = _seed()
        seed["execution_allowed_now"] = True

        errors = validate_seed_payload(seed, "broken_seed", REPO_ROOT)

        self.assertTrue(any("execution_allowed_now" in error for error in errors))

    def test_seed_with_accepted_runtime_workunit_true_fails(self) -> None:
        seed = _seed()
        seed["accepted_as_runtime_workunit"] = True

        errors = validate_seed_payload(seed, "broken_seed", REPO_ROOT)

        self.assertTrue(any("accepted_as_runtime_workunit" in error for error in errors))
    ...
    def test_conversion_executing_workunit_fails(self) -> None:
        conversion = _conversion()
        conversion["execution_allowed_now"] = True

        errors = validate_conversion_payload(conversion, "broken_conversion", REPO_ROOT)

        self.assertTrue(any("execution_allowed_now" in error for error in errors))

    def test_conversion_accepting_runtime_workunit_fails(self) -> None:
        conversion = _conversion()
        conversion["accepted_as_runtime_workunit"] = True
    ...
    def test_live_probe_or_api_call_claim_fails(self) -> None:
        conversion = _conversion()
        conversion["notes"] = ["API call completed for this seed."]

        errors = validate_conversion_payload(conversion, "broken_conversion", REPO_ROOT)

        self.assertTrue(any("api call completed" in error for error in errors))
    ...
    def test_source_approval_claim_fails(self) -> None:
        conversion = _conversion()
        conversion["notes"] = ["source approval granted"]
    ...
    def test_script_does_not_create_observed_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            output_path = Path(temp) / "workunits.json"
    ...
    def test_scripts_do_not_mutate_pending_observations(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            self.assertEqual(build_main(["--repo-root", str(REPO_ROOT), "--json-output", str(Path(temp) / "workunits.json")], stdout=io.StringIO()), 0)
...
def _manifest() -> dict:
    return deepcopy(_read_json(REPO_ROOT / "control/inventory/observations/workunit_seed_manifest.json"))
