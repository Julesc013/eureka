...
from scripts import validate_h7_library_research_fixture_runtime as validator

REPO_ROOT = Path(__file__).resolve().parents[2]


class H7LibraryResearchFixtureScriptTests(unittest.TestCase):
    ...
    def test_normalizer_writes_no_files_by_default(self) -> None:
        before = {path.as_posix() for path in (REPO_ROOT / "examples/connectors/h7_library_research/normalized").glob("*.json")}
        ...
        after = {path.as_posix() for path in (REPO_ROOT / "examples/connectors/h7_library_research/normalized").glob("*.json")}
        self.assertEqual(before, after)

    def test_normalizer_writes_explicit_outputs_to_temp_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "normalized.json"
            bibliographic = Path(tmp) / "bibliographic.json"
            result = subprocess.run([
                sys.executable, "scripts/normalize_h7_library_research_fixture.py",
                "--source-id", "openalex",
                "--input", "examples/connectors/h7_library_research/fixtures/openalex/identity_record.json",
                "--output", str(output),
                "--bibliographic-output", str(bibliographic),
            ], cwd=REPO_ROOT, text=True, capture_output=True, check=False)
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertEqual(json.loads(output.read_text(encoding="utf-8"))["source_id"], "openalex")
            self.assertIn("candidate_id", json.loads(bibliographic.read_text(encoding="utf-8")))

    def test_replay_writes_no_files_by_default_and_temp_when_explicit(self) -> None:
        result = subprocess.run([sys.executable, "scripts/replay_h7_library_research_fixtures.py", "--check"], cwd=REPO_ROOT, text=True, capture_output=True, check=False)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        with tempfile.TemporaryDirectory() as tmp:
            result = subprocess.run([sys.executable, "scripts/replay_h7_library_research_fixtures.py", "--source-id", "openalex", "--output-dir", tmp], cwd=REPO_ROOT, text=True, capture_output=True, check=False)
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_summary_check(self) -> None:
        result = subprocess.run([sys.executable, "scripts/summarize_h7_library_research_fixture_outputs.py", "--input", "examples/connectors/h7_library_research", "--check", "--json"], cwd=REPO_ROOT, text=True, capture_output=True, check=False)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertEqual(json.loads(result.stdout)["source_count"], 30)
