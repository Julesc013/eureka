...
from scripts import summarize_h6_web_archive_news_event_sources as summary_script


ROOT = Path(__file__).resolve().parents[2]


class H6WebArchiveNewsEventSummaryTests(unittest.TestCase):
    def test_build_summary_counts_sources(self) -> None:
        ...
        self.assertEqual(summary["fetch_enabled_count"], 0)
    ...
    def test_cli_check_writes_no_files_by_default(self) -> None:
        before = {path.as_posix() for path in (ROOT / "control/audits/h6-bundle-01-web-archive-news-event-policy-packs-v0/generated").glob("*")}
        result = subprocess.run(
            [sys.executable, "scripts/summarize_h6_web_archive_news_event_sources.py", "--check"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
        ...
        after = {path.as_posix() for path in (ROOT / "control/audits/h6-bundle-01-web-archive-news-event-policy-packs-v0/generated").glob("*")}
    ...
    def test_json_mode(self) -> None:
        result = subprocess.run(
            [sys.executable, "scripts/summarize_h6_web_archive_news_event_sources.py", "--check", "--json"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=False,
