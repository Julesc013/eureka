...
from scripts.record_search_miss import main as record_search_miss_main
from scripts.validate_search_miss_ledger_runtime import validate_search_miss_ledger_runtime
...
class SearchMissLedgerRuntimeScriptTests(unittest.TestCase):
    def test_script_writes_no_files_by_default(self) -> None:
        ...
        result = record_search_miss_main(["--input", str(EMPTY_QUERY_OBSERVATION), "--check"], stdout=stdout)
    ...
    def test_script_writes_explicit_report_to_temp_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "search_miss_report.json"
            stdout = io.StringIO()
            result = record_search_miss_main(
                ["--input", str(EMPTY_QUERY_OBSERVATION), "--output", str(output), "--json"],
                stdout=stdout,
            )
        ...
        self.assertEqual(payload["schema_version"], "search_miss_ledger_runtime_report.v0")

    def test_script_refuses_site_dist_output(self) -> None:
        forbidden = ROOT / "site" / "dist" / "__search_miss_forbidden_report.json"
        if forbidden.exists():
            forbidden.unlink()
    ...
    def test_script_refuses_runtime_output(self) -> None:
        forbidden = ROOT / "runtime" / "__search_miss_forbidden_report.json"
        if forbidden.exists():
            forbidden.unlink()
    ...
    def test_validator_passes_current_repo(self) -> None:
        report = validate_search_miss_ledger_runtime(ROOT)
    ...
    def test_runtime_script_does_not_call_network_model_or_provider(self) -> None:
        with mock.patch("socket.create_connection", side_effect=AssertionError("network call blocked")):
            stdout = io.StringIO()
            result = record_search_miss_main(["--input", str(EMPTY_QUERY_OBSERVATION), "--check", "--json"], stdout=stdout)
