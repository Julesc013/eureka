...
from scripts.validate_h3_os_package_archive_fixture_runtime import validate_repo


REPO_ROOT = Path(__file__).resolve().parents[2]


class H3OSPackageFixtureScriptTests(unittest.TestCase):
    ...
    def test_normalizer_script_writes_no_files_by_default(self):
        result = self.run_cmd([
            ...
            "debian_snapshot",
            "--input",
            "examples/connectors/h3_os_package_archives/fixtures/debian_snapshot/typical_record.json",
        ])
    ...
    def test_normalizer_script_writes_explicit_outputs_to_temp_paths(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertEqual(json.loads(out.read_text())["schema_version"], "h3_os_package_normalized_record.v0")
    ...
    def test_replay_script_writes_no_files_by_default(self):
        result = self.run_cmd(["scripts/replay_h3_os_package_fixtures.py", "--check"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("fixture_replay_count: 78", result.stdout)

    def test_replay_script_writes_explicit_outputs_to_temp_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            result = self.run_cmd(["scripts/replay_h3_os_package_fixtures.py", "--source-id", "debian_snapshot", "--output-dir", tmp])
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_scripts_refuse_forbidden_roots(self):
        ...
        bad_private = self.run_cmd([
            ...
            "examples/connectors/h3_os_package_archives/fixtures/debian_snapshot/typical_record.json",
            "--output",
            ".local/eureka/h3.json",
        ])
        self.assertNotEqual(bad_private.returncode, 0)

    def test_summary_script_passes(self):
        result = self.run_cmd(["scripts/summarize_h3_os_package_fixture_outputs.py", "--input", "examples/connectors/h3_os_package_archives", "--check", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
