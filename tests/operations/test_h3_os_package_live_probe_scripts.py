...
RUNNER = REPO_ROOT / "scripts/run_h3_os_package_live_probe.py"
SUMMARY = REPO_ROOT / "scripts/summarize_h3_os_package_live_probe_outputs.py"
VALIDATOR = REPO_ROOT / "scripts/validate_h3_os_package_live_probe.py"
GENERATED = REPO_ROOT / "control/audits/h3-bundle-03-os-package-live-probes-v0/generated"
...
class H3OSPackageLiveProbeScriptTests(unittest.TestCase):
    def test_cli_writes_no_files_by_default(self):
        before = sorted(path.name for path in GENERATED.glob("*"))
        result = run_cmd(["scripts/run_h3_os_package_live_probe.py", "--source-id", "debian_snapshot", "--request-key", "example_package_metadata"])
    ...
    def test_cli_live_without_approval_is_blocked_offline(self):
        result = run_cmd(["scripts/run_h3_os_package_live_probe.py", "--source-id", "debian_snapshot", "--request-key", "example_package_metadata", "--live", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_cli_writes_explicit_outputs_to_temp_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertEqual(json.loads((tmp_path / "result.json").read_text(encoding="utf-8"))["source_id"], "debian_snapshot")
            self.assertTrue((tmp_path / "summary.md").is_file())

    def test_cli_refuses_site_dist_output(self):
        result = run_cmd(["scripts/run_h3_os_package_live_probe.py", "--source-id", "debian_snapshot", "--request-key", "example_package_metadata", "--output", "site/dist/h3.json"])
    ...
    def test_cli_refuses_repository_mirror_output(self):
        result = run_cmd(["scripts/run_h3_os_package_live_probe.py", "--source-id", "debian_snapshot", "--request-key", "example_package_metadata", "--output", "repository_mirror/h3.json"])
        self.assertNotEqual(result.returncode, 0)

    def test_summary_script_passes_on_examples(self):
        result = run_cmd(["scripts/summarize_h3_os_package_live_probe_outputs.py", "--input", "examples/connectors/h3_os_package_archives/live_probe_results", "--check", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        payload = json.loads(result.stdout)
        self.assertFalse(payload["network_used"])
        self.assertFalse(payload["repository_index_sync_used"])
        self.assertGreaterEqual(payload["blocked_count"], 13)

    def test_validator_passes_current_repo(self):
        result = run_cmd(["scripts/validate_h3_os_package_live_probe.py", "--json"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        payload = json.loads(result.stdout)
    ...
    def test_validator_does_not_create_local_private_roots(self):
        result = run_cmd(["scripts/validate_h3_os_package_live_probe.py"])
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
