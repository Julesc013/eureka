...
class H8ManualsDocsLiveProbeScriptTests(unittest.TestCase):
    ...
    def test_cli_writes_no_files_by_default(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            run = self.run_cmd(["scripts/run_h8_manuals_docs_live_probe.py", "--source-id", "bitsavers_docs", "--request-key", "example_document_metadata", "--json"])
    ...
    def test_cli_writes_explicit_outputs_to_temp_path(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            run = self.run_cmd(["scripts/run_h8_manuals_docs_live_probe.py", "--source-id", "bitsavers_docs", "--request-key", "example_document_metadata", "--output", str(out), "--summary-output", str(summary), "--json"])
            self.assertEqual(run.returncode, 0, run.stdout + run.stderr)
    ...
    def test_summary_script_writes_explicit_outputs(self):
        with tempfile.TemporaryDirectory() as tempdir:
            out = Path(tempdir) / "summary.json"
            md = Path(tempdir) / "summary.md"
            run = self.run_cmd(["scripts/summarize_h8_manuals_docs_live_probe_outputs.py", "--input", "examples/connectors/h8_manuals_docs_standards/live_probe_results", "--output", str(out), "--summary-output", str(md), "--json"])
            self.assertEqual(run.returncode, 0, run.stdout + run.stderr)
            self.assertTrue(out.is_file())
    ...
    def test_validator_passes_and_uses_no_private_roots(self):
        run = self.run_cmd(["scripts/validate_h8_manuals_docs_live_probe.py", "--json"])
        self.assertEqual(run.returncode, 0, run.stdout + run.stderr)
