...
INTEGRATE = REPO_ROOT / "scripts/integrate_h1_metadata_review.py"
QUALITY = REPO_ROOT / "scripts/summarize_h1_quality_delta.py"
VALIDATOR = REPO_ROOT / "scripts/validate_h1_review_quality_audit.py"


class H1ReviewQualityScriptsTest(unittest.TestCase):
    ...
    def test_scripts_write_no_files_by_default(self):
        before = sorted(path.name for path in (REPO_ROOT / "examples/connectors/h1_metadata_wave/review_integration").glob("*.json"))
        result = self.run_integrate("--input-dir", "examples/connectors/h1_metadata_wave/replay_results", "--json")
        after = sorted(path.name for path in (REPO_ROOT / "examples/connectors/h1_metadata_wave/review_integration").glob("*.json"))
    ...
    def test_scripts_write_explicit_outputs_to_temp_paths(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertTrue((out_dir / "h1_review_integration_result_v0.json").is_file())
            quality = self.run_quality(
                "--input-dir",
                str(out_dir),
                "--output",
                str(Path(tmp) / "quality.json"),
                "--summary-output",
                str(Path(tmp) / "quality.md"),
            )
            self.assertEqual(quality.returncode, 0, quality.stdout + quality.stderr)
            self.assertFalse(json.loads((Path(tmp) / "quality.json").read_text(encoding="utf-8"))["truth_boundary"]["claims_production_readiness"])
