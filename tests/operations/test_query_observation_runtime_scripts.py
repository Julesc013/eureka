...
from scripts.record_query_observation import main as record_query_observation_main
from scripts.validate_query_observation_runtime import validate_query_observation_runtime
...
class QueryObservationRuntimeScriptTests(unittest.TestCase):
    def test_script_writes_no_files_by_default(self) -> None:
        ...
        result = record_query_observation_main(["--input", str(MINIMAL), "--check"], stdout=stdout)
    ...
    def test_script_writes_explicit_report_to_temp_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "query_observation_report.json"
            stdout = io.StringIO()
            result = record_query_observation_main(
                ["--input", str(MINIMAL), "--output", str(output), "--json"],
                stdout=stdout,
            )
        ...
        self.assertEqual(payload["schema_version"], "query_observation_runtime_report.v0")

    def test_script_refuses_site_dist_output(self) -> None:
        forbidden = ROOT / "site" / "dist" / "__query_observation_forbidden_report.json"
        if forbidden.exists():
            forbidden.unlink()
    ...
    def test_script_refuses_runtime_output(self) -> None:
        forbidden = ROOT / "runtime" / "__query_observation_forbidden_report.json"
        if forbidden.exists():
            forbidden.unlink()
    ...
    def test_validator_passes_current_repo(self) -> None:
        report = validate_query_observation_runtime(ROOT)
    ...
    def test_runtime_script_does_not_call_network_model_or_provider(self) -> None:
        with mock.patch("socket.create_connection", side_effect=AssertionError("network call blocked")):
            stdout = io.StringIO()
            result = record_query_observation_main(["--input", str(MINIMAL), "--check", "--json"], stdout=stdout)
