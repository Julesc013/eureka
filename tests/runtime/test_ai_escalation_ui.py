...
class AIEscalationUiTests(unittest.TestCase):
    def test_hunt_and_need_pages_show_disabled_boundary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("AI escalation gate", hunt_html)
                self.assertIn("AI escalation gate", need_html)
                self.assertIn("provider_enabled", hunt_html)
                self.assertIn("candidate_only_output", need_html)
                self.assertNotIn("call model provider", hunt_html.lower())
                self.assertNotIn("execute", hunt_html.lower())
            finally:
