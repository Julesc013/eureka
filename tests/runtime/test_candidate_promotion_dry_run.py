...
def build_from_candidate(candidate_name: str = "search_need_candidate_v0.json", *, evidence: list[dict] | None = None, review: list[dict] | None = None) -> dict:
    return promotion.build_candidate_promotion_dry_run(
        {
            "candidate": load_json(f"examples/candidates/{candidate_name}"),
...
class CandidatePromotionDryRunRuntimeTests(unittest.TestCase):
    def test_build_promotion_dry_run_works_on_candidate_example(self) -> None:
        record = build_from_candidate(review=[load_json("examples/review_queue_entries/candidate_needs_review_v0.json")])
    ...
    def test_candidate_with_review_and_fixture_evidence_can_be_ready(self) -> None:
        record = build_from_candidate(
            evidence=[load_json("examples/evidence_ledger_records/metadata_claim_record_v0.json")],
            review=[load_json("examples/review_queue_entries/workunit_result_review_v0.json")],
        )
    ...
    def test_candidate_missing_evidence_is_blocked(self) -> None:
        record = build_from_candidate(review=[load_json("examples/review_queue_entries/workunit_result_review_v0.json")])
        self.assertEqual(record["promotion_readiness"], "not_ready_missing_evidence")
    ...
    def test_candidate_missing_review_is_blocked(self) -> None:
        record = build_from_candidate(evidence=[load_json("examples/evidence_ledger_records/metadata_claim_record_v0.json")])
    ...
    def test_conflict_detected_candidate_is_blocked(self) -> None:
        candidate = load_json("examples/candidates/search_need_candidate_v0.json")
        candidate["candidate_status"] = "conflict_detected"
        record = promotion.build_candidate_promotion_dry_run(
            {
                "candidate": candidate,
                "evidence_records": [load_json("examples/evidence_ledger_records/metadata_claim_record_v0.json")],
                "review_entries": [load_json("examples/review_queue_entries/workunit_result_review_v0.json")],
            }
        )
    ...
    def test_ready_for_promotion_dry_run_does_not_accept_candidate_or_evidence(self) -> None:
        record = promotion.build_candidate_promotion_dry_run(load_json("examples/candidate_promotion_dry_runs/ready_for_promotion_dry_run_v0.json"))
    ...
    def test_rights_malware_installability_and_exhaustive_claims_are_rejected(self) -> None:
        record = promotion.build_candidate_promotion_dry_run(load_json("examples/candidate_promotion_dry_runs/minimal_promotion_dry_run_v0.json"))
    ...
    def test_duplicate_and_conflict_are_not_auto_resolved(self) -> None:
        record = promotion.build_candidate_promotion_dry_run(load_json("examples/candidate_promotion_dry_runs/duplicate_blocked_promotion_dry_run_v0.json"))
    ...
    def test_runtime_does_not_import_network_model_or_provider_modules(self) -> None:
        source = (REPO_ROOT / "runtime/local_foundry/candidate_promotion_dry_run.py").read_text(encoding="utf-8")
