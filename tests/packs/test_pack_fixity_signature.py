...
from runtime.local_foundry import pack_fixity, pack_quarantine, pack_signature
...
class PackFixitySignatureTests(unittest.TestCase):
    def test_sha256_fixity_is_deterministic(self) -> None:
        pack = load_json("examples/pack_exports/evidence_pack_export_v0.json")
    ...
    def test_fixity_does_not_imply_authenticity(self) -> None:
        report = pack_fixity.build_pack_fixity_report(load_json("examples/pack_exports/evidence_pack_export_v0.json"))
    ...
    def test_unsigned_pack_produces_needs_review_status(self) -> None:
        envelope = pack_signature.parse_pack_signature_envelope({"pack_export_id": "pack.unsigned.test.v0", "signature_envelope": load_json("examples/pack_quarantine/signatures/unsigned_pack_signature_envelope_v0.json")})
    ...
    def test_placeholder_signature_envelope_validates_as_placeholder_only(self) -> None:
        envelope = load_json("examples/pack_quarantine/signatures/placeholder_signature_envelope_v0.json")
    ...
    def test_malformed_signature_envelope_blocks_acceptance(self) -> None:
        envelope = load_json("examples/pack_quarantine/signatures/malformed_signature_envelope_v0.json")
        report = pack_signature.build_signature_verification_report(envelope)
