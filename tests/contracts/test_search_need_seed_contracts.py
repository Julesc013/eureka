...
class SearchNeedSeedContractsTest(unittest.TestCase):
    def test_seed_contract_json_is_valid_and_declares_boundaries(self) -> None:
        payload = _read_json(REPO_ROOT / "contracts/query/search_need_seed.v0.json")

        errors = validate_seed_contract_payload(payload, "seed_contract")

        self.assertEqual(errors, [])
        self.assertFalse(payload["x-runtime-search-need-created"])
    ...
    def test_conversion_contract_json_is_valid_and_declares_boundaries(self) -> None:
        payload = _read_json(REPO_ROOT / "contracts/query/search_need_seed_conversion.v0.json")
    ...
    def test_seed_truth_boundary_flags_fail_when_true(self) -> None:
    ...
    def test_conversion_truth_boundary_flags_fail_when_true(self) -> None:
