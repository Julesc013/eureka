...
EXAMPLE = ROOT / "examples/connectors/pypi_metadata_approval_v0/PYPI_METADATA_CONNECTOR_APPROVAL.json"


class PyPIMetadataConnectorApprovalValidatorTests(unittest.TestCase):
    def test_approval_validator_passes(self):
        c = subprocess.run(
            [sys.executable, "scripts/validate_pypi_metadata_connector_approval.py", "--all-examples"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=True,
    ...
    def test_approval_validator_json_parses(self):
        c = subprocess.run(
            [sys.executable, "scripts/validate_pypi_metadata_connector_approval.py", "--all-examples", "--json"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=True,
    ...
    def test_negative_runtime_mutation_and_source_calls_fail(self):
        cases = [
            ...
            ("pypi_api_called", ["no_runtime_guarantees", "pypi_api_called"]),
            ("package_metadata_fetched", ["no_runtime_guarantees", "package_metadata_fetched"]),
            ("wheels_downloaded", ["no_runtime_guarantees", "wheels_downloaded"]),
            ("sdists_downloaded", ["no_runtime_guarantees", "sdists_downloaded"]),
    ...
    def test_negative_package_policy_contact_and_secret_fail(self):
        ...
        self._assert_invalid(p)
        p = copy.deepcopy(base)
        p["notes"].append("pypi_token=not-a-real-token")
        self._assert_invalid(p)
        p = copy.deepcopy(base)
    ...
    def test_negative_output_runtime_and_forbidden_policy_fail(self):
        ...
        for item in p["forbidden_capabilities"]:
            if item["capability"] == "wheel_download":
        ...
        self._assert_invalid(p)
        p = copy.deepcopy(base)
        p["rights_access_and_risk_policy"]["dependency_safety_claimed"] = True
        self._assert_invalid(p)
        p = copy.deepcopy(base)
        p["rights_access_and_risk_policy"]["installability_claimed"] = True
        self._assert_invalid(p)
    ...
    def _assert_invalid(self, payload):
        with tempfile.TemporaryDirectory() as td:
            ...
            c = subprocess.run(
                [sys.executable, "scripts/validate_pypi_metadata_connector_approval.py", "--approval", str(path), "--json"],
                cwd=ROOT,
                text=True,
                capture_output=True,
            )
