...
class SearchMissLedgerEntryValidatorTests(unittest.TestCase):
    def test_all_examples_pass(self) -> None:
        completed = subprocess.run(
            [sys.executable, "scripts/validate_search_miss_ledger_entry.py", "--all-examples"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=True,
    ...
    def test_all_examples_json_parses(self) -> None:
        completed = subprocess.run(
            [sys.executable, "scripts/validate_search_miss_ledger_entry.py", "--all-examples", "--json"],
            cwd=ROOT,
            text=True,
            capture_output=True,
            check=True,
    ...
    def test_search_need_creation_fails(self) -> None:
        payload = json.loads(EXAMPLE.read_text(encoding="utf-8"))
        payload["no_mutation_guarantees"]["search_need_created"] = True
        report = self._validate_temp(payload)
        self.assertEqual(report["status"], "invalid")
        self.assertTrue(any("search_need_created" in error for error in report["errors"]))
    ...
    def test_global_absence_phrase_fails(self) -> None:
        payload = json.loads(EXAMPLE.read_text(encoding="utf-8"))
        payload["absence_summary"]["limitations"] = ["Scoped check says every source was checked."]
        report = self._validate_temp(payload)
        self.assertEqual(report["status"], "invalid")
    ...
    def _validate_temp(self, payload: dict) -> dict:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "SEARCH_MISS_LEDGER_ENTRY.json"
            path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
            completed = subprocess.run(
                [sys.executable, "scripts/validate_search_miss_ledger_entry.py", "--entry", str(path), "--json"],
                cwd=ROOT,
                text=True,
                capture_output=True,
            )
