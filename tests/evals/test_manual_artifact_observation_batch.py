...
class ManualArtifactObservationBatchTests(unittest.TestCase):
    def test_observation_package_loads_and_covers_all_hard_queries(self) -> None:
        ...
        for item in observations:
            self.assertTrue(item["artifact_level"])
    ...
    def test_artifact_level_and_status_counts_match_observations(self) -> None:
        ...
        self.assertEqual(
            artifact_level_counts(observations),
            {
                ...
                "artifact_level_1_metadata_or_source_lead": 1,
                "artifact_level_2_source_observed_artifact_listing": 4,
                "artifact_level_3_artifact_identity_evidence": 5,
                "artifact_level_4_artifact_integrity_evidence": 0,
                "artifact_level_5_verified_acquisition_or_reproducibility_path": 0,
            },
        )
        self.assertEqual(summary["manual_artifact_observation_count"], 11)
        self.assertEqual(summary["reviewable_artifact_item_count"], 10)
    ...
    def test_query_mapping_reviewable_items_and_sources_validate(self) -> None:
        ...
        self.assertEqual(len(reviewable_item_records(reviewable)), 10)
        self.assertEqual(len(source_reference_records(load_source_reference_index())), 11)
    ...
    def test_public_alpha_artifact_gate_remains_blocked(self) -> None:
        ...
        self.assertEqual(gate["manual_artifact_observation_count"], 11)
        self.assertEqual(gate["reviewable_artifact_item_count"], 10)
        self.assertTrue(gate["public_alpha_blocked"])
        self.assertTrue(gate["dev_to_main_blocked"])
        self.assertEqual(gate["next_recommended_task"], "HUMAN-ARTIFACT-REVIEW-BATCH-00")
