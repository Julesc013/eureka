...
class HumanArtifactReviewBatchOneTests(unittest.TestCase):
    ...
    def test_review_decision_counts_preserve_truth_boundary(self) -> None:
        ...
        for decision in decisions:
            ...
            self.assertIn("verified artifact", " ".join(decision["what_must_not_be_claimed_publicly"]))

        self.assertEqual(len(decisions), 6)
        self.assertEqual(decision_counts["promote"], 2)
        self.assertEqual(decision_counts["request_more_evidence"], 3)
        self.assertEqual(decision_counts["mark_near_miss"], 1)
    ...
    def test_promote_decisions_have_review_events_and_reviewed_record_ids(self) -> None:
        ...
        for decision in promote_decisions:
            ...
            self.assertTrue(event["created_reviewed_artifact_record"])
    ...
    def test_non_promoted_outcomes_do_not_create_records(self) -> None:
        non_promoted = [item for item in outcome_records(load_review_decision_backed_outcomes()) if item["decision"] != "promote"]

        self.assertEqual(len(non_promoted), 4)
        for outcome in non_promoted:
            self.assertFalse(outcome["reviewed_artifact_record_created"])
            self.assertIsNone(outcome["reviewed_artifact_record_id"])
            self.assertFalse(outcome["verified_artifact_created"])
            self.assertIn(outcome["outcome_status"], {"need", "near_miss"})
