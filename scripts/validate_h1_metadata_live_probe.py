...
PYTHON_FILES = tuple(
    ["control/prototypes/legacy_runtime/connectors/h1_metadata_wave/live_probe_common.py"]
    + [f"control/prototypes/legacy_runtime/connectors/h1_metadata_wave/live_probe_{source_id}.py" for source_id in H1_SOURCE_IDS]
    + [
...
def validate_cli_offline(root: Path, errors: list[str]) -> None:
    ...
    bad_public = subprocess.run([sys.executable, "scripts/run_h1_metadata_live_probe.py", "--source-id", "pypi", "--request-key", "example_project_metadata", "--output", "data/public_index/h1.json"], cwd=root, check=False, capture_output=True, text=True, timeout=120)
