#!/usr/bin/env python3
"""Summarize H10 games/emulation live-probe outputs without network use."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        inputs = args.input or ["examples/connectors/h10_games_emulation/live_probe_results"]
        ...
        if args.json:
            ...
            print("H10 games emulation live probe output summary", file=stdout)
...
def load_results(inputs: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for item in inputs:
        ...
        for file in files:
            ...
            if isinstance(payload, Mapping) and payload.get("schema_version") == "h10_games_emulation_live_probe_result.v0":
...
def summarize(results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "h10_games_emulation_live_probe_output_summary.v0",
        "status": "valid",
        ...
        "game_software_identity_candidate_count": sum(1 for item in results if _candidate_present(item.get("game_software_identity_candidate"))),
        "platform_release_edition_candidate_count": sum(1 for item in results if _candidate_present(item.get("platform_release_edition_candidate"))),
        "emulator_compatibility_candidate_count": sum(1 for item in results if _candidate_present(item.get("emulator_compatibility_candidate"))),
        "preservation_hashset_candidate_count": sum(1 for item in results if _candidate_present(item.get("preservation_hashset_candidate"))),
        "rom_disc_media_identity_candidate_count": sum(1 for item in results if _candidate_present(item.get("rom_disc_media_identity_candidate"))),
        "game_relation_candidate_count": sum(1 for item in results if item.get("game_relation_candidate") and not _blocked(item.get("game_relation_candidate"))),
        "emulator_action_candidate_count": sum(1 for item in results if _candidate_present(item.get("emulator_action_candidate"))),
        "rights_safety_candidate_count": sum(1 for item in results if _candidate_present(item.get("games_rights_safety_candidate"))),
...
def render_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        "# H10 Games Emulation Live Probe Output Summary",
        "",
