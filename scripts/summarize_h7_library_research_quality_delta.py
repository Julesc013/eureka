#!/usr/bin/env python3
"""Summarize explicit H7 library/cultural/research review integration outputs."""
...
from control.prototypes.legacy_runtime.connectors.h7_library_research.quality_delta import (  # noqa: E402
    build_h7_quality_delta,
    detect_h7_quality_overclaim,
    summarize_h7_quality_delta,
)


def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", action="append", default=[], help="Explicit H7 integration JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H7 integration JSON files. May be repeated.")
    ...
    try:
        ...
        summary = summarize_h7_quality_delta(delta)
        ...
        if args.json:
            ...
            print("H7 library/cultural/research quality delta", file=stdout)
...
def build_quality_delta(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    review = next((payload for payload in payloads if payload.get("schema_version") == "h7_library_research_review_integration_result.v0"), None)
    if review is None:
        raise ValueError("no h7_library_research_review_integration_result.v0 input found")
    delta = build_h7_quality_delta({"review_integration_result": review})
    errors = detect_h7_quality_overclaim(delta)
    if errors:
