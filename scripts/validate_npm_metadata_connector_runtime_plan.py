#!/usr/bin/env python3
"""Validate npm Metadata Connector Runtime Planning v0 artifacts."""
...
AUDIT_DIR = REPO_ROOT / "control" / "audits" / "npm-metadata-connector-runtime-planning-v0"
REPORT_PATH = AUDIT_DIR / "npm_metadata_connector_runtime_planning_report.json"
INVENTORY_PATH = REPO_ROOT / "control" / "inventory" / "connectors" / "npm_metadata_connector_runtime_plan.json"
DOC_PATH = REPO_ROOT / "docs" / "operations" / "NPM_METADATA_CONNECTOR_RUNTIME_PLAN.md"
...
REQUIRED_FILES = {
    ...
    "NEXT_STEPS.md",
    "npm_metadata_connector_runtime_planning_report.json",
}
...
VALID_READINESS = {
    ...
    "blocked_scoped_package_policy_missing",
    "blocked_dependency_metadata_policy_missing",
    "blocked_lifecycle_script_policy_missing",
    "blocked_token_auth_policy_missing",
    "blocked_package_download_install_dependency_script_boundary_missing",
...
INVENTORY_FALSE_FIELDS = {
    "runtime_connector_implemented",
    "live_calls_enabled",
    "npm_registry_api_calls_enabled",
    "npm_cli_enabled",
    "source_sync_worker_execution_enabled",
    "source_cache_runtime_enabled",
    "evidence_ledger_runtime_enabled",
    ...
    "version_metadata_fetch_enabled",
    "dist_tag_metadata_fetch_enabled",
    "tarball_metadata_fetch_enabled",
    "tarball_download_enabled",
    "package_file_download_enabled",
    "package_install_enabled",
    "dependency_resolution_enabled",
...
INVENTORY_TRUE_FIELDS = {
    ...
    "lifecycle_script_policy_review_required",
...
def validate() -> list[str]:
    ...
    if REPORT_PATH.exists():
        ...
        if not isinstance(loaded, Mapping):
            ...
            gates = loaded.get("package_identity_scoped_package_source_policy_gate_review", {})
            if not isinstance(gates, Mapping):
                errors.append("report.package_identity_scoped_package_source_policy_gate_review must be an object")
            else:
                for field in (
                    "package_identity_review_required",
                    "scoped_package_policy_review_required",
                    ...
                    if gates.get(field) is not True:
                        errors.append(f"report.package_identity_scoped_package_source_policy_gate_review.{field} must be true")
            ...
            if not isinstance(dep, Mapping):
                ...
                for field in (
                    "dependency_resolution_enabled",
                    "dependency_graph_expansion_enabled",
            ...
            lifecycle = loaded.get("lifecycle_script_risk_boundary_review", {})
            if not isinstance(lifecycle, Mapping):
                errors.append("report.lifecycle_script_risk_boundary_review must be an object")
            else:
                for field in (
                    "lifecycle_script_execution_enabled",
                    "preinstall_install_postinstall_execution_enabled",
                    "npm_audit_enabled",
                    "script_safety_claimed",
                    "package_execution_claimed",
                ):
                    if lifecycle.get(field) is not False:
                        errors.append(f"report.lifecycle_script_risk_boundary_review.{field} must be false")
            ...
            if not isinstance(token, Mapping):
                ...
                if token.get("npm_token_enabled") is not False:
                    errors.append("report.token_auth_boundary_review.npm_token_enabled must be false")
    ...
    if INVENTORY_PATH.exists():
        ...
        if not isinstance(loaded, Mapping):
            ...
            if loaded.get("connector_id") != "npm_metadata_connector":
                errors.append("inventory.connector_id must be npm_metadata_connector")
    ...
    if AUDIT_DIR.exists():
        require_phrases(
            ...
            (
                "does not implement",
                "no live npm calls",
                "no npm registry endpoints are called",
                "no package metadata, version metadata, dist-tag metadata, or tarball metadata is fetched",
                "no tarballs or package files are downloaded",
                "no npm/yarn/pnpm commands are invoked",
                "no dependency resolution",
                "no npm audit",
                "no lifecycle scripts are executed",
                "no source-cache records",
                "no evidence-ledger records",
                "no credentials or npm tokens",
                "no indexes are mutated",
            ),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "PACKAGE_IDENTITY_SCOPED_PACKAGE_AND_SOURCE_POLICY_GATE_REVIEW.md",
            ("package identity", "scoped package", "source policy", "operator approval", "pending", "does not fabricate"),
            errors,
        )
        require_phrases(
            ...
            (
                ...
                "vulnerability/security claims are forbidden",
                "must not trigger live fetches",
                "peer, optional, dev, and bundled dependencies",
            ),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "LIFECYCLE_SCRIPT_RISK_BOUNDARY_REVIEW.md",
            (
                "lifecycle script metadata",
                "script execution remains disabled",
                "preinstall, install, and postinstall execution remain disabled",
                "npm audit remains disabled",
                "script safety claims are forbidden",
                "must not trigger package download",
            ),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "TOKEN_AUTH_BOUNDARY_REVIEW.md",
            ("token-free", "npm token", "not configured", "future explicit policy"),
            errors,
        )
        require_phrases(
        ...
        require_phrases(
            ...
            (
                ...
                "package-name public-safe fingerprint",
                "no raw private package/registry publication",
            ),
            errors,
        )
        require_phrases(
    ...
    if DOC_PATH.exists():
        text = require_phrases(
            ...
            (
                ...
                "no npm audit",
                "no mutation",
                "public search must not call npm live",
            ),
            errors,
            "operations doc",
        )
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    if args.json:
        payload = {
            "status": status,
            "report_id": "npm_metadata_connector_runtime_planning_v0",
