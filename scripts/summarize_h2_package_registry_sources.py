#!/usr/bin/env python3
"""Summarize H2 package-registry sources and policy-pack posture offline."""
...
SOURCE_INVENTORY = REPO_ROOT / "control/inventory/source_packs/h2_package_registry_sources.json"
CONNECTOR_FAMILIES = REPO_ROOT / "control/inventory/source_packs/h2_package_registry_connector_families.json"
IDENTITY_POLICY = REPO_ROOT / "control/inventory/source_packs/h2_package_registry_identity_policy.json"
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("H2 package registry source summary", file=stdout)
            ...
            print(f"install_execute_enabled_count: {summary['install_execute_enabled_count']}", file=stdout)
        return 0
...
def build_summary() -> dict[str, Any]:
    ...
    if not isinstance(sources, list):
        raise ValueError("H2 source inventory must contain a sources list")
    ...
    package_identity_count = sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("package_identity_support"), Mapping))
    ...
    return {
        "schema_version": "h2_package_registry_source_summary.v0",
        "status": "pass",
        "wave_id": inventory.get("wave_id", "H2"),
        ...
        "package_identity_support_count": package_identity_count,
        ...
        "readiness": "READY_FOR_H2_FIXTURE_RUNTIME",
        "truth_boundary": {
...
def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    lines = [
        "# H2 Package Registry Source Summary",
        "",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/source_packs/") or rel_lower.startswith("examples/connectors/h2_package_registries/"):
            return resolved
        raise ValueError(f"refusing output outside approved H2 roots: {rel}")
