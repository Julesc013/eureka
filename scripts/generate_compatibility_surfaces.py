...
def _build_lite_surface(data: Mapping[str, Mapping[str, Any]]) -> dict[str, str]:
    ...
    index_body = [
        ...
        "</ul>",
        "<p>Compatibility Surface Strategy v0 governs these projections as static views of the same resolver truth. Signed Snapshot Format v0 adds a repo seed example only. Relay Surface Design v0 records future local/LAN relay policy only. Neither milestone publishes a public /snapshots/ route, production signed snapshots, relay runtime, native clients, live API behavior, or runtime product behavior.</p>",
    ...
    limitations_body = [
        ...
        *(f"<li>{escape(str(item))}</li>" for item in limitations),
        ...
        "<li>Public /snapshots/ routes, production signed snapshots, relay runtime, native clients, and live APIs remain future work.</li>",
        "<li>Relay Surface Design v0 is contract-only; no relay server, protocol bridge, network listener, private data exposure, or write/admin path exists here.</li>",
...
def _build_text_surface(data: Mapping[str, Mapping[str, Any]]) -> dict[str, str]:
    ...
    index = _text_page(
        ...
        [
            ...
            "Relay Surface Design v0 records a future local/LAN bridge contract only; no relay runtime or protocol server exists.",
            "",
            "Files:",
            "- sources.txt",
            "- evals.txt",
    ...
    limitation_lines = [
        ...
        "- relay surface design contract only; no relay server or protocol implementation",
...
def _build_files_surface(data: Mapping[str, Mapping[str, Any]]) -> dict[str, str]:
    ...
    manifest = {
        ...
        "relay_surface_contract": "docs/reference/RELAY_SURFACE_CONTRACT.md",
        "relay_surface_design_status": "contract_only_runtime_absent",
    ...
    index_txt = _text_page(
        ...
        [
            ...
            "Relay Surface Design v0: contract only; no relay runtime, protocol server, network listener, private data exposure, or write/admin path.",
    ...
    index_html = _lite_html(
        ...
        [
            ...
            "<p>Relay Surface Design v0 records future local/LAN relay policy only. It does not add a relay server, protocol bridge, network listener, private data exposure, live probe passthrough, or write/admin route.</p>",
            "<ul>",
