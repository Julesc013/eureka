#!/usr/bin/env python3
"""Build a descriptive acquisition manifest without fetching or downloading."""
...
from runtime.actions.acquisition_manifest import build_acquisition_manifest, validate_acquisition_manifest  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        manifest = build_acquisition_manifest(subject, policy)
        errors = validate_acquisition_manifest(manifest, policy)
        if errors:
            for error in errors:
        ...
        if args.output and not args.check:
            ...
            wrote_files = True
        response = {"schema_version": "acquisition_manifest_cli_result.v0", "status": "pass", "wrote_files": wrote_files, "acquisition_manifest": manifest}
        if args.json:
            print(json.dumps(response, indent=2, sort_keys=True))
        else:
            print(f"Acquisition manifest: {manifest['acquisition_manifest_id']}\nDownloads: false\nMirrors: false\n")
        return 0
