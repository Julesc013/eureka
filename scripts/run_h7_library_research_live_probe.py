#!/usr/bin/env python3
"""Run or preflight H7 library/research metadata live probes with fail-closed policy gates."""
...
from control.prototypes.legacy_runtime.connectors.h7_library_research.live_probe_common import (  # noqa: E402
    SOURCE_CONFIGS,
    build_h7_library_research_live_probe_blocked_result,
    build_h7_library_research_live_probe_output_bundle,
    build_h7_library_research_live_probe_request,
    load_h7_library_research_live_probe_policy_bundle,
    summarize_h7_library_research_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", help="Optional explicit H7 live-probe request envelope.")
    ...
    try:
        ...
        if not args.check:
            outputs = {
                args.output: result,
                args.bibliographic_output: result["bibliographic_identity_candidate"],
                args.research_work_output: result["research_work_identity_candidate"],
                args.dataset_output: result["dataset_identity_candidate"],
                args.cultural_object_output: result["cultural_object_identity_candidate"],
                args.patent_output: result["patent_identity_candidate"],
                args.citation_output: result["citation_relation_candidate"],
                args.access_output: result["access_rights_availability_candidate"],
        ...
        summary = {
            ...
            "wrote_files": (not args.check) and any([
                args.output,
                args.bibliographic_output,
                args.research_work_output,
                args.dataset_output,
                args.cultural_object_output,
                args.patent_output,
                args.citation_output,
                args.access_output,
            ...
            "live_probe": summarize_h7_library_research_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H7 library/research metadata live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    validation = validate_h7_library_research_live_probe_request(request, policy_bundle)
    if not validation["approved"]:
        result = build_h7_library_research_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h7_library_research_live_probe_output_bundle(result)}
    if not live:
        result = build_h7_library_research_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h7_library_research_live_probe_output_bundle(result)}
    result = build_h7_library_research_live_probe_blocked_result(
        request,
        ["live network execution remains fail-closed in H7-BUNDLE-03; source-specific executor requires future reviewed approval"],
        policy_bundle,
    )
...
def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h7_library_research_live_probe_result(result)
    lines = [
        "# H7 Library Research Live Probe Summary",
        "",
        ...
        "- oai_pmh_harvest: `false`",
        "- api_query: `false unless approved bounded metadata-only`",
        "- downloads: `false`",
        "- scraping_crawling: `false`",
        "- restricted_source_access: `false`",
        f"- bibliographic_candidate_present: `{str(summary['bibliographic_candidate_present']).lower()}`",
        f"- research_work_candidate_present: `{str(summary['research_work_candidate_present']).lower()}`",
        f"- dataset_candidate_present: `{str(summary['dataset_candidate_present']).lower()}`",
        f"- cultural_object_candidate_present: `{str(summary['cultural_object_candidate_present']).lower()}`",
        f"- patent_candidate_present: `{str(summary['patent_candidate_present']).lower()}`",
        f"- citation_candidate_present: `{str(summary['citation_candidate_present']).lower()}`",
        f"- access_rights_candidate_present: `{str(summary['access_rights_candidate_present']).lower()}`",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h7_library_research_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h7_library_research/live_probe_results/"):
            return resolved
        if rel_lower.startswith("examples/connectors/h7_library_research/live_probe_outputs/"):
            return resolved
        raise ValueError(f"refusing output outside approved H7 live-probe roots: {rel}")
