#!/usr/bin/env python3
"""Validate SEED-BATCH-MANUALS-SCANS-00."""
...
REQUIRED_POLICIES = [
    "control/policies/seed_batch_manuals_scans_policy.json",
    "control/policies/seed_batch_manuals_scans_query_policy.json",
    "control/policies/seed_batch_manuals_scans_candidate_policy.json",
    "control/policies/seed_batch_manuals_scans_review_policy.json",
    "control/policies/seed_batch_manuals_scans_suppression_policy.json",
    "control/policies/seed_batch_manuals_scans_non_claim_policy.json",
    "control/policies/seed_batch_manuals_scans_live_metadata_policy.json",
]
REQUIRED_MATRICES = [
    "control/inventory/seed_batch_manuals_scans_input_state.json",
    "control/inventory/seed_batch_manuals_scans_query_matrix.json",
    "control/inventory/seed_batch_manuals_scans_source_plan_matrix.json",
    "control/inventory/seed_batch_manuals_scans_candidate_matrix.json",
    "control/inventory/seed_batch_manuals_scans_suppression_matrix.json",
    "control/inventory/seed_batch_manuals_scans_scout_matrix.json",
    "control/inventory/seed_batch_manuals_scans_review_matrix.json",
    "control/inventory/seed_batch_manuals_scans_need_absence_matrix.json",
    "control/inventory/seed_batch_manuals_scans_snapshot_handoff_matrix.json",
    "control/inventory/seed_batch_manuals_scans_public_alpha_reassess_matrix.json",
    "control/inventory/seed_batch_manuals_scans_boundary_report.json",
    "control/inventory/seed_batch_manuals_scans_smoke_result.json",
    "control/inventory/seed_batch_manuals_scans_result.json",
    "control/inventory/seed_batch_manuals_scans_next_task_decision.json",
    "control/inventory/seed_batch_manuals_scans_failure_repair_log.json",
]
REQUIRED_EXAMPLES = [
    "examples/seed_batches/manuals_scans/query_set.json",
    "examples/seed_batches/manuals_scans/query_plans.json",
    "examples/seed_batches/manuals_scans/source_plans.json",
    "examples/seed_batches/manuals_scans/suppressions.json",
    "examples/seed_batches/manuals_scans/candidate_summaries.json",
    "examples/seed_batches/manuals_scans/candidate_index.json",
    "examples/seed_batches/manuals_scans/scout_trails.json",
    "examples/seed_batches/manuals_scans/review_batch_packet.json",
    "examples/seed_batches/manuals_scans/known_needs.json",
    "examples/seed_batches/manuals_scans/absence_summaries.json",
    "examples/seed_batches/manuals_scans/snapshot_refresh_handoff.json",
    "examples/seed_batches/manuals_scans/public_alpha_reassess_input.json",
    "examples/seed_batches/manuals_scans/boundary_report.json",
    "examples/query_plans/manuals_scans/query_plans.json",
    "examples/candidates/manuals_scans/candidate_summaries.json",
    "examples/candidates/manuals_scans/candidate_index.json",
    "examples/scout/manuals_scans/scout_trails.json",
    "examples/review_batch/manuals_scans/review_batch_packet.json",
    "examples/public_alpha/manuals_scans/public_alpha_reassess_input.json",
]
REQUIRED_DOCS = [
...
REQUIRED_QUERIES = [item["raw_query"] for item in MANUALS_SCANS_QUERIES]
REQUIRED_SUPPRESSIONS = [item["suppression_id"] for item in MANUALS_SCANS_SUPPRESSIONS]
...
def validate() -> dict[str, Any]:
    ...
    return {
        "schema_version": "seed_batch_manuals_scans_validation.v0",
...
def _runtime_checks() -> dict[str, bool]:
    ...
    snapshot_handoff = build_manuals_scans_snapshot_refresh_handoff(review_packets)
    public_alpha = build_manuals_scans_public_alpha_reassess_inputs(result)
    boundary = result["boundary_report"]
    candidate_summaries = result["candidate_summaries"]
    inventory = build_manuals_scans_inventory_packets(result)
    return {
        ...
        "inventory_packets_build": "seed_batch_manuals_scans_result.json" in inventory,
        ...
        "no_download_fetch_ocr_extract_model_deploy": all(result.get(key) is False for key in BOUNDARY_FALSE if key in result),
        "no_document_quality_or_rights_claims": all(
            result.get(key) is False
            for key in ("rights_clearance_claim_created", "scan_completeness_claim_created", "ocr_quality_claim_created")
        ),
    }
...
def _query_matrix_has_required_queries() -> bool:
    path = "control/inventory/seed_batch_manuals_scans_query_matrix.json"
    if not (REPO_ROOT / path).exists():
        return False
    payload = _load_json(path)
    queries = [item.get("raw_query") for item in payload.get("queries", [])]
    return queries == REQUIRED_QUERIES and all(
        item.get("domain_id") == "manuals_docs_scans" and item.get("accepted_truth") is False
        for item in payload.get("queries", [])
    )


def _suppression_matrix_has_required_suppressions() -> bool:
    path = "control/inventory/seed_batch_manuals_scans_suppression_matrix.json"
    if not (REPO_ROOT / path).exists():
        return False
    payload = _load_json(path)
...
def _source_plan_matrix_safe() -> bool:
    path = "control/inventory/seed_batch_manuals_scans_source_plan_matrix.json"
    if not (REPO_ROOT / path).exists():
        return False
    payload = _load_json(path)
    ...
    return (
        ...
        and all(item.get("file_fetch_enabled") is False for item in families.values())
