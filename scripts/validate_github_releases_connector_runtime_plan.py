#!/usr/bin/env python3
"""Validate GitHub Releases Connector Runtime Planning v0 artifacts."""
...
AUDIT_DIR = REPO_ROOT / "control" / "audits" / "github-releases-connector-runtime-planning-v0"
REPORT_PATH = AUDIT_DIR / "github_releases_connector_runtime_planning_report.json"
INVENTORY_PATH = REPO_ROOT / "control" / "inventory" / "connectors" / "github_releases_connector_runtime_plan.json"
DOC_PATH = REPO_ROOT / "docs" / "operations" / "GITHUB_RELEASES_CONNECTOR_RUNTIME_PLAN.md"
...
REQUIRED_FILES = {
    ...
    "NEXT_STEPS.md",
    "github_releases_connector_runtime_planning_report.json",
}
...
def validate() -> list[str]:
    ...
    if REPORT_PATH.exists():
        ...
        if not isinstance(loaded, Mapping):
            ...
            gates = loaded.get("repository_identity_source_policy_gate_review", {})
            if not isinstance(gates, Mapping):
                errors.append("report.repository_identity_source_policy_gate_review must be an object")
            else:
                for field in ("repository_identity_review_required", "source_policy_review_required", "operator_approval_required"):
                    if gates.get(field) is not True:
                        errors.append(f"report.repository_identity_source_policy_gate_review.{field} must be true")
            ...
            if not isinstance(token, Mapping):
                ...
                if token.get("github_token_enabled") is not False:
                    errors.append("report.token_auth_boundary_review.github_token_enabled must be false")
            ...
            for phrase in ("approval", "repository identity", "token/auth", "source policy", "user-agent/contact", "rate limits", "cache", "evidence attribution", "kill switch", "operator approval"):
    ...
    if INVENTORY_PATH.exists():
        ...
        if not isinstance(loaded, Mapping):
            ...
            if loaded.get("connector_id") != "github_releases_connector":
                errors.append("inventory.connector_id must be github_releases_connector")
    ...
    if AUDIT_DIR.exists():
        require_phrases(
            ...
            (
                ...
                "no releases, tags, repository metadata",
                "no repositories are cloned",
                "no source-cache records",
                "no evidence-ledger records",
                "no credentials or github tokens",
                "no indexes are mutated",
            ),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "REPOSITORY_IDENTITY_AND_SOURCE_POLICY_GATE_REVIEW.md",
            ("repository identity", "source policy", "operator approval", "pending", "does not fabricate"),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "TOKEN_AUTH_BOUNDARY_REVIEW.md",
            ("token-free", "github token", "not configured", "future explicit approval"),
            errors,
        )
        require_phrases(
        ...
        require_phrases(
            AUDIT_DIR / "REPOSITORY_IDENTITY_REVIEW_AND_NORMALIZATION_PLAN.md",
            ("reviewed source record", "raw public query parameter", "private repository", "credentialed repository", "owner/repo public-safe fingerprint", "no raw private repository publication"),
            errors,
        )
        require_phrases(
        ...
        require_phrases(
            ...
            ("p73 approval complete", "repository identity policy reviewed", "token/auth boundary reviewed", "source policy reviewed", "user-agent/contact configured", "rate limits approved", "cache destination", "evidence attribution", "kill switch", "operator approval"),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "DO_NOT_IMPLEMENT_YET.md",
            ("no github api calls", "no connector runtime", "no source cache writes", "no evidence ledger writes", "no public search live fanout", "no arbitrary repository fetch", "no repository clone", "no release asset download", "no source archive download", "no raw blob/file fetch", "no index/master mutation"),
            errors,
        )
    ...
    if DOC_PATH.exists():
        text = require_phrases(
            DOC_PATH,
            ("planning-only", "no runtime", "no external calls", "no arbitrary repository", "no asset/source download", "no mutation", "public search must not call github live"),
            errors,
            "operations doc",
        )
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    if args.json:
        payload = {
            "status": status,
            "report_id": "github_releases_connector_runtime_planning_v0",
