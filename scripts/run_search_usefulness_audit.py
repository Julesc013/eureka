...
def main(argv: Sequence[str] | None = None, *, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run Eureka's bounded Search Usefulness Audit v0 over the current "
            "repo-level query pack. External baselines are not scraped and remain "
    ...
    parser.add_argument(
        "--query",
        dest="query_id",
        help="Optional search-usefulness query id to run by itself.",
    )
    parser.add_argument(
        "--index-path",
    ...
    suite = runner.run_suite(query_id=args.query_id, index_path=args.index_path)
    ...
    if args.json:
        ...
        output.write(format_search_usefulness_audit_summary(suite))
    return 0
