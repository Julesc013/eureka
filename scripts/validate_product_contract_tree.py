    ...
    for source in (reference_result.get("blocked_moves", []) or []) + (previous_migration.get("blocked", []) or []):
        ...
        if path and (root / path).exists() and path in contract_by_path:
            unresolved[path] = {
                ...
                "severity": "high",
                "recommended_next_action": "Update the active consumer in a task that is allowed to touch that path, then move the schema to control/schemas/.",
            }
...
def render_operation_doc(result: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "It moved the remaining safe control and task schemas out of `contracts/` and into `control/schemas/`.",
            "",
