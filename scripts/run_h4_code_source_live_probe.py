#!/usr/bin/env python3
"""Run or preflight H4 code/source/release metadata live probes with fail-closed policy gates."""
...
from runtime.connectors.h4_code_source_release.live_probe_common import (  # noqa: E402
    H4CodeSourceLiveProbeBlocked,
    SOURCE_CONFIGS,
    build_h4_code_source_live_probe_blocked_result,
    build_h4_code_source_live_probe_output_bundle,
    build_h4_code_source_live_probe_request,
    build_h4_code_source_live_probe_result,
    fetch_h4_code_source_metadata_once,
    load_h4_code_source_live_probe_policy_bundle,
    summarize_h4_code_source_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", help="Optional explicit H4 code/source live-probe request envelope.")
    parser.add_argument("--output")
    parser.add_argument("--source-identity-output")
    parser.add_argument("--release-identity-output")
    ...
    try:
        ...
        if not args.check:
            ...
            if args.source_identity_output:
                _write_json(args.source_identity_output, result["source_identity_candidate"])
            if args.release_identity_output:
                _write_json(args.release_identity_output, result["release_identity_candidate"])
            if args.relation_output:
                _write_json(args.relation_output, {"source_to_binary_relation_candidate_preview": result["source_to_binary_relation_candidate_preview"]})
            if args.asset_output:
                _write_json(args.asset_output, {"release_asset_candidate_preview": result["release_asset_candidate_preview"]})
        ...
        summary = {
            ...
            "live_probe": summarize_h4_code_source_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H4 code/source/release metadata live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    ...
    if not validation["approved"]:
        result = build_h4_code_source_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h4_code_source_live_probe_output_bundle(result)}
    if not live:
        result = build_h4_code_source_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h4_code_source_live_probe_output_bundle(result)}
    try:
        payload, metadata = fetch_h4_code_source_metadata_once(request, policy_bundle)
    except H4CodeSourceLiveProbeBlocked as exc:
        result = exc.result
        return {"live_probe_result": result, "output_bundle": build_h4_code_source_live_probe_output_bundle(result)}
    result = build_h4_code_source_live_probe_result(str(request["source_id"]), payload, metadata, policy_bundle)
    return {"live_probe_result": result, "output_bundle": build_h4_code_source_live_probe_output_bundle(result)}


def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h4_code_source_live_probe_result(result)
    lines = [
        "# H4 Code/Source/Release Metadata Live Probe Summary",
        "",
        ...
        f"- repository_clone_used: `{str(summary['repository_clone_used']).lower()}`",
        f"- source_archive_download_used: `{str(summary['source_archive_download_used']).lower()}`",
        f"- release_asset_download_used: `{str(summary['release_asset_download_used']).lower()}`",
        f"- relation_candidate_count: `{summary['relation_candidate_count']}`",
        f"- release_asset_candidate_count: `{summary['release_asset_candidate_count']}`",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h4_code_source_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h4_code_source_release/live_probe_results/"):
            return resolved
