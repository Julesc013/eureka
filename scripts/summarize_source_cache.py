#!/usr/bin/env python3
"""Summarize explicit fixture-only source cache records."""
...
from scripts import record_source_cache  # noqa: E402
...
def build_report(input_paths: Sequence[Path], *, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    for path in expanded:
        ...
        if payload.get("schema_version") == source_cache.SNAPSHOT_SCHEMA_VERSION:
            records.extend(record for record in payload.get("records", []) if isinstance(record, dict))
        elif payload.get("schema_version") == source_cache.REPORT_SCHEMA_VERSION and isinstance(payload.get("record"), dict):
            records.append(payload["record"])
        else:
            records.append(payload)
    snapshot = source_cache.build_source_cache_snapshot(records)
    summary = source_cache.summarize_source_cache(snapshot["records"])
...
def write_report(report: Mapping[str, Any], output_path: Path) -> None:
    if not record_source_cache.output_path_allowed(output_path):
...
def write_summary(report: Mapping[str, Any], output_path: Path) -> None:
    if not record_source_cache.output_path_allowed(output_path):
        raise ValueError(f"refusing forbidden output path: {output_path}")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(source_cache.format_source_cache_summary_markdown(report["summary"]), encoding="utf-8")


def main(argv: Sequence[str] | None = None, *, stdout: TextIO | None = None, stderr: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", required=True, action="append", type=Path, help="Source cache record JSON file or directory.")
    ...
    try:
        ...
        err.write(f"summarize_source_cache: ERROR: {exc}\n")
        return 2
    ...
    if args.json:
        ...
        out.write("Source cache summary\n")
        out.write(f"status: {report['status']}\n")
        out.write(f"source_cache_record_count: {summary.get('source_cache_record_count')}\n")
...
def _expand_inputs(input_paths: Sequence[Path]) -> list[Path]:
    ...
    if not paths:
        raise ValueError("no source cache JSON inputs found")
    return paths
