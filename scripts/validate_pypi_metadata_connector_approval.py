#!/usr/bin/env python3
"""Validate PyPI Metadata Connector Approval v0 examples."""
...
APPROVAL_FILE = "PYPI_METADATA_CONNECTOR_APPROVAL.json"
MANIFEST_FILE = "PYPI_METADATA_CONNECTOR_MANIFEST.json"
...
ALLOWED = {
    "package_project_metadata_future",
    "package_release_metadata_future",
    "package_file_metadata_summary_future",
    "requires_python_metadata_future",
    "classifier_metadata_future",
    "yanked_status_metadata_future",
    "package_summary_description_metadata_future",
    "dependency_metadata_summary_future",
}
FORBID = {
    "arbitrary_package_fetch",
    "direct_public_query_fanout",
    "private_package_index_access",
...
SCOPE_ALLOWED = {
    ...
    "release_metadata_future",
    "file_metadata_summary_future",
}
SCOPE_FORBID = {
    "arbitrary_public_query_package",
    "private_package",
    "credentialed_package_index",
    "alternate_package_index",
    "package_file_download",
    "wheel_download",
    "sdist_download",
    "package_install",
...
EVID_OUT = {
    ...
    "package_file_metadata_observation",
    "requires_python_observation",
    "classifier_observation",
    "yanked_status_observation",
...
OPERATOR_ITEMS = {
    ...
    "confirm_no_package_download_or_install",
...
BAD_SECRET = re.compile(
    r"\b(?:api[_-]?key|auth[_-]?token|pypi[_-]?token|password|secret)\s*[:=]",
    re.IGNORECASE,
)
BAD_PACKAGE = re.compile(
    r"\b(?:private|credentialed|token-required)/(?:package|project)\b|"
    r"\b(?:file|data|javascript):|https?://[^\s/]*@|pypi\.org|pythonhosted",
    re.IGNORECASE,
)
...
def validate_approval(p: Mapping[str, Any], errors: list[str]) -> None:
    require_fields(p, REQ, errors, "pypi_metadata_connector_approval")
    if p.get("approval_record_kind") != "pypi_metadata_connector_approval":
        errors.append("approval_record_kind must be pypi_metadata_connector_approval.")
    check_allowed(p.get("status"), STAT, errors, "status")
    ref = sec(p, "connector_ref", errors)
    if ref.get("connector_id") != "pypi_metadata_connector":
        errors.append("connector_ref.connector_id must be pypi_metadata_connector.")
    if ref.get("source_family") != "pypi":
        errors.append("connector_ref.source_family must be pypi.")
    ...
    if source.get("review_status") != "review_required":
        errors.append("source_policy_review.review_status must be review_required in P74 examples.")
    ...
    require_true(cache, {"source_cache_required_before_public_use", "evidence_ledger_required_for_claims", "public_search_reads_cache_not_live_source_future", "public_query_fanout_forbidden", "cache_invalidation_required_future", "freshness_policy_required_future", "no_package_payload_cache", "no_wheel_payload_cache", "no_sdist_payload_cache", "no_installation_artifact_cache"}, errors, "cache_first_policy")

    validate_outputs(p.get("expected_source_cache_outputs"), CACHE_OUT, {"output_runtime_implemented", "raw_payload_allowed", "package_payload_allowed", "wheel_payload_allowed", "sdist_payload_allowed"}, {"public_safe_metadata_only", "requires_validation"}, "expected_source_cache_outputs", errors)
    validate_outputs(p.get("expected_evidence_ledger_outputs"), EVID_OUT, {"output_runtime_implemented", "accepted_as_truth", "global_absence_claimed", "installability_claimed", "dependency_safety_claimed"}, {"requires_review", "requires_promotion_policy"}, "expected_evidence_ledger_outputs", errors)
    ...
    require_false(public, {"public_search_live_fanout_allowed", "public_search_may_read_live_connector_now", "public_search_may_accept_arbitrary_package_param", "static_site_claim_allowed_now", "hosted_backend_claim_allowed_now"}, errors, "public_search_boundary")

    rights = sec(p, "rights_access_and_risk_policy", errors)
    require_false(rights, {"rights_clearance_claimed", "malware_safety_claimed", "dependency_safety_claimed", "installability_claimed", "downloads_enabled", "file_retrieval_enabled", "package_file_download_enabled", "wheel_download_enabled", "sdist_download_enabled", "package_install_enabled", "dependency_resolution_enabled", "installs_enabled", "execution_enabled"}, errors, "rights_access_and_risk_policy")
    ...
    if not isinstance(access, list) or not {"metadata_only", "no_package_file_download", "no_wheel_download", "no_sdist_download", "no_install", "no_private_indexes"}.issubset(set(access)):
    ...
    require_false(privacy, {"private_data_allowed", "credentials_required_now", "account_access_allowed", "contains_private_path", "contains_secret", "contains_private_package", "contains_private_url"}, errors, "privacy_policy")
    if privacy.get("privacy_classification") not in {"public_safe_metadata_policy", "package_identity_review_required", "review_required", "unknown"}:
    ...
    for key in ("arbitrary_package_fetch_allowed", "package_install_allowed", "wheel_download_allowed", "sdist_download_allowed", "package_file_download_allowed", "dependency_resolution_allowed_now", "token_required_now", "dependency_safety_claimed", "installability_claimed"):
...
def validate_manifest(m: Mapping[str, Any], errors: list[str]) -> None:
    ...
    require_true(d, {"package_metadata_only_scope", "package_identity_review_required", "dependency_metadata_caution_required", "token_policy_review_required", "no_token_required_now", "no_package_download_or_install", "approval_required_for_live_network_access", "source_policy_review_required", "cache_first_required", "evidence_attribution_required"}, errors, "manifest.default_policy")
    ...
    require_true(safety, {"no_downloads", "no_file_retrieval", "no_mirroring", "no_arbitrary_package_fetch", "no_package_install", "no_wheel_download", "no_sdist_download", "no_package_file_download", "no_dependency_resolution", "no_package_archive_inspection", "no_public_query_fanout", "no_credentials", "no_pypi_token", "no_telemetry"}, errors, "manifest.safety_defaults")
...
def validate_package_policy(v: Mapping[str, Any], errors: list[str]) -> None:
    require_true(v, {"package_name_review_required", "normalized_package_name_publication_allowed_after_review", "package_name_hashing_allowed_future", "package_name_redaction_required_for_sensitive_inputs"}, errors, "package_identity_policy")
    require_false(v, {"arbitrary_public_query_package_allowed", "private_package_allowed", "credentialed_package_index_allowed", "token_required_package_allowed", "alternate_index_allowed_now", "local_package_path_allowed", "raw_package_url_publication_allowed"}, errors, "package_identity_policy")
    ...
    require_list(v.get("prohibited_package_name_sources"), {"raw_public_query_parameter", "user_private_package", "credentialed_package_index", "local_path", "uploaded_file"}, "package_identity_policy.prohibited_package_name_sources", errors)


def validate_dependency_policy(v: Mapping[str, Any], errors: list[str]) -> None:
    require_true(v, {"dependency_metadata_allowed_as_summary_future", "requires_python_metadata_allowed_future", "dependency_names_publication_requires_review"}, errors, "dependency_metadata_caution_policy")
    require_false(v, {"dependency_resolution_allowed_now", "dependency_graph_expansion_allowed_now", "installability_claim_allowed", "security_or_vulnerability_claim_allowed", "dependency_safety_claim_allowed"}, errors, "dependency_metadata_caution_policy")
...
def validate_all_examples(strict: bool = False) -> dict[str, Any]:
    ...
    if not results:
        errors.append("no PyPI metadata connector approval examples found.")
    return {
        "status": "valid" if not errors else "invalid",
        "created_by": "pypi_metadata_connector_approval_validator_v0",
...
def check_forbidden_strings(payload: Any, errors: list[str], prefix: str) -> None:
    ...
    if BAD_PACKAGE.search(text):
        errors.append(f"{prefix} contains prohibited private, credentialed, local, hosted-file, or API package reference.")
