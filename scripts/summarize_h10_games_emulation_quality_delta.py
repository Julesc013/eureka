#!/usr/bin/env python3
"""Summarize explicit H10 games/emulation review integration outputs."""
...
from control.prototypes.legacy_runtime.connectors.h10_games_emulation.quality_delta import (  # noqa: E402
    build_h10_quality_delta,
    detect_h10_quality_overclaim,
    summarize_h10_quality_delta,
)


def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", action="append", default=[], help="Explicit H10 integration JSON file. May be repeated.")
    parser.add_argument("--input-dir", action="append", default=[], help="Directory containing H10 integration JSON files. May be repeated.")
    ...
    try:
        ...
        summary = summarize_h10_quality_delta(delta)
        ...
        if args.json:
            ...
            print("H10 games/emulation quality delta", file=stdout)
...
def build_quality_delta(inputs: Sequence[str], input_dirs: Sequence[str]) -> dict[str, Any]:
    ...
    review = next((payload for payload in payloads if payload.get("schema_version") == "h10_games_emulation_review_integration_result.v0"), None)
    if review is None:
        raise ValueError("no h10_games_emulation_review_integration_result.v0 input found")
    delta = build_h10_quality_delta({"review_integration_result": review})
    errors = detect_h10_quality_overclaim(delta)
    if errors:
...
def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    return "\n".join([
        ...
        "- rom_authenticity: `false`",
        "- disc_authenticity: `false`",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("examples/connectors/h10_games_emulation/review_integration/"):
            return resolved
