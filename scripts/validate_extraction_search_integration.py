...
def validate_scripts(root: Path, errors: list[str]) -> None:
    ...
    public = subprocess.run(
        [sys.executable, "scripts/summarize_extraction_search_gaps.py", "--input", "examples/extraction/search_integration", "--output", "data/public_index/extraction.json"],
        cwd=root,
        capture_output=True,
        text=True,
        timeout=120,
    ...
    if public.returncode == 0 or "refusing forbidden output root" not in public.stdout:
        errors.append("search gap summary script must reject data/public_index output")
