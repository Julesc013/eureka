"""Summarize WorkUnit seed draft candidates without mutating them."""
...
DEFAULT_MANIFEST_PATH = "control/inventory/observations/workunit_seed_manifest.json"


def main(argv: Sequence[str] | None = None, *, stdout: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(description="Summarize WorkUnit seed draft candidates.")
...
def summarize_seed_manifest(manifest_path: Path, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    policy_state_counts = Counter(str(record.get("source_policy_status")) for record in records)
    ...
    for record in sorted(records, key=lambda item: str(item.get("workunit_seed_id"))):
        grouped[_priority_band(record.get("proposed_priority"))].append(str(record.get("workunit_seed_id")))
    return {
        "schema_version": "workunit_seed_summary.v0",
        ...
        "by_policy_state": dict(sorted(policy_state_counts.items())),
...
def format_plain_summary(report: Mapping[str, Any]) -> str:
    lines = [
        "summarize_workunit_seed_candidates:",
        ...
        f"- by_policy_state: {json.dumps(report.get('by_policy_state', {}), sort_keys=True)}",
...
def format_markdown_summary(report: Mapping[str, Any]) -> str:
    lines = [
        "# WorkUnit Seed Candidate Summary",
        "",
        f"WorkUnit seed drafts: {report.get('seed_count')}",
        "",
        "## Priority Bands",
        "",
    ]
    ...
    lines.extend(f"- `{key}`: {value}" for key, value in _mapping(report.get("by_policy_state")).items())
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
            "- This summary does not approve WorkUnit seeds.",
            "- This summary does not create or execute runtime WorkUnits.",
