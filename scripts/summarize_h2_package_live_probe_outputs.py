#!/usr/bin/env python3
"""Summarize H2 package live-probe result files without network access."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("H2 package live probe output summary", file=stdout)
...
def load_results(inputs: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for item in inputs:
        ...
        for json_path in paths:
            ...
            if isinstance(payload, dict) and payload.get("schema_version") == "h2_package_live_probe_result.v0":
...
def build_summary(results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "h2_package_live_probe_output_summary.v0",
        "status": "pass",
        ...
        "identity_candidate_count": sum(1 for item in results if isinstance(item.get("package_identity_candidate"), Mapping) and item.get("package_identity_candidate", {}).get("status") != "not_created_blocked_by_policy"),
...
def render_summary(summary: Mapping[str, Any]) -> str:
    lines = [
        "# H2 Package Live Probe Output Summary",
        "",
