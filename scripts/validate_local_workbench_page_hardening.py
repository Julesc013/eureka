...
RUNTIME_FILES = (
    "runtime/local_workbench/__init__.py",
    ...
    "runtime/local_workbench/errors.py",
)
...
def validate_queue(root: Path, errors: list[str]) -> None:
    ...
    if "current_recommended_task: LOCAL-07" not in queue:
        errors.append("queue index must point to LOCAL-07")
    if "id: LOCAL-06" not in queue or "status: completed" not in queue:
        errors.append("queue index must mark LOCAL-06 completed")
    if "id: LOCAL-07" not in queue or "status: queued" not in queue:
