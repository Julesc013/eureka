#!/usr/bin/env python3
"""Replay committed H9 media metadata fixtures offline."""
...
from control.prototypes.legacy_runtime.connectors.h9_media_metadata.normalizer_common import H9_SOURCE_IDS, build_h9_fixture_replay_result  # noqa: E402
from scripts.normalize_h9_media_metadata_fixture import safe_output_path  # noqa: E402

ALLOWED_PREFIXES = ("examples/connectors/h9_media_metadata/replay_results",)


def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--source-id", choices=H9_SOURCE_IDS)
    parser.add_argument("--fixture-root", default="examples/connectors/h9_media_metadata/fixtures")
    ...
    try:
        ...
        summary = {
            "schema_version": "h9_media_metadata_fixture_replay_summary.v0",
            "status": "pass",
        ...
        if args.json:
            ...
            print("H9 media metadata fixture replay", file=stdout)
...
def replay_fixtures(fixture_root: str | Path, source_id: str | None = None) -> list[dict[str, Any]]:
    ...
    for item in ([source_id] if source_id else list(H9_SOURCE_IDS)):
        assert item is not None
        source_dir = root / item
        if not source_dir.is_dir():
            raise ValueError(f"missing fixture directory: {source_dir}")
        module = importlib.import_module(f"control.prototypes.legacy_runtime.connectors.h9_media_metadata.{item}")
        for fixture_path in sorted(source_dir.glob("*.json")):
            fixture = load_h9_media_metadata_fixture(fixture_path)
            normalized = module.normalize(fixture)
            results.append(build_h9_fixture_replay_result(fixture, normalized))
    return results
