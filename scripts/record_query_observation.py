#!/usr/bin/env python3
"""Record a local-only Query Observation report from explicit JSON input.
...
from runtime.local_foundry.query_observation import (  # noqa: E402
...
def build_report(input_path: Path, *, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    record = build_query_observation(payload)
...
def main(argv: Sequence[str] | None = None, *, stdout: TextIO | None = None, stderr: TextIO | None = None) -> int:
    ...
    parser.add_argument("--input", required=True, type=Path, help="Explicit query observation JSON input.")
    ...
    try:
        ...
        err.write(f"record_query_observation: ERROR: {exc}\n")
        return 2
    ...
    if args.json:
        ...
        out.write("Query Observation report\n")
        out.write(f"status: {report['status']}\n")
        out.write(f"query_observation_id: {summary.get('query_observation_id')}\n")
        out.write(f"outcome_classification: {summary.get('outcome_classification')}\n")
