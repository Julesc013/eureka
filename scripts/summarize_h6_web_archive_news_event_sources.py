#!/usr/bin/env python3
"""Summarize H6 web archive/news/event policy-pack sources offline."""
...
SOURCE_INVENTORY = REPO_ROOT / "control/inventory/source_packs/h6_web_archive_news_event_sources.json"
CONNECTOR_FAMILIES = REPO_ROOT / "control/inventory/source_packs/h6_web_archive_news_event_connector_families.json"
WEB_CAPTURE_POLICY = REPO_ROOT / "control/inventory/source_packs/h6_web_capture_identity_policy.json"
TIME_STATE_POLICY = REPO_ROOT / "control/inventory/source_packs/h6_archived_url_time_state_policy.json"
NEWS_EVENT_POLICY = REPO_ROOT / "control/inventory/source_packs/h6_news_event_mention_policy.json"
DEAD_LINK_POLICY = REPO_ROOT / "control/inventory/source_packs/h6_dead_link_trace_policy.json"
PUBLIC_DOCUMENT_POLICY = REPO_ROOT / "control/inventory/source_packs/h6_public_document_trace_policy.json"
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("H6 web archive/news/event source summary", file=stdout)
            ...
            print(f"fetch_enabled_count: {summary['fetch_enabled_count']}", file=stdout)
            print(f"scraping_crawling_enabled_count: {summary['scraping_crawling_enabled_count']}", file=stdout)
            print(f"restricted_source_enabled_count: {summary['restricted_source_enabled_count']}", file=stdout)
        return 0
...
def build_summary() -> dict[str, Any]:
    ...
    web_capture_policy = _load_json(WEB_CAPTURE_POLICY)
    time_state_policy = _load_json(TIME_STATE_POLICY)
    news_event_policy = _load_json(NEWS_EVENT_POLICY)
    dead_link_policy = _load_json(DEAD_LINK_POLICY)
    public_document_policy = _load_json(PUBLIC_DOCUMENT_POLICY)
    sources = inventory.get("sources", [])
    if not isinstance(sources, list):
        raise ValueError("H6 source inventory must contain a sources list")
    ...
    fetch_keys = ("cdx_query_enabled", "memento_lookup_enabled", "warc_wacz_fetch_enabled", "archived_page_fetch_enabled", "media_download_enabled", "public_document_fetch_enabled")
    fetch_enabled_count = sum(1 for item in sources if isinstance(item, Mapping) and any(item.get(key) is True for key in fetch_keys))
    ...
    return {
        "schema_version": "h6_web_archive_news_event_source_summary.v0",
        "status": "pass",
        "wave_id": inventory.get("wave_id", "H6"),
        ...
        "web_capture_identity_support_count": sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("web_capture_identity_support"), Mapping) and item["web_capture_identity_support"].get("capture_metadata_future") is True),
        "archived_url_time_state_support_count": sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("archived_url_time_state_support"), Mapping) and item["archived_url_time_state_support"].get("time_state_metadata_future") is True),
        "news_event_mention_support_count": sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("news_event_mention_support"), Mapping) and item["news_event_mention_support"].get("mention_metadata_future") is True),
        "dead_link_trace_support_count": sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("dead_link_trace_support"), Mapping) and item["dead_link_trace_support"].get("dead_link_trace_future") is True),
        "public_document_trace_support_count": sum(1 for item in sources if isinstance(item, Mapping) and isinstance(item.get("public_document_trace_support"), Mapping) and item["public_document_trace_support"].get("public_document_trace_future") is True),
        "sensitive_or_restricted_source_count": sum(1 for item in sources if isinstance(item, Mapping) and str(item.get("sensitive_or_restricted_source_posture", "")).startswith("policy_blocked")),
        "web_capture_concepts": web_capture_policy.get("identity_concepts", []),
        "time_state_concepts": time_state_policy.get("time_state_concepts", []),
        "news_event_concepts": news_event_policy.get("mention_event_concepts", []),
        "dead_link_concepts": dead_link_policy.get("trace_concepts", []),
        "public_document_concepts": public_document_policy.get("public_document_concepts", []),
        ...
        "fetch_enabled_count": fetch_enabled_count,
        "scraping_crawling_enabled_count": scraping_crawling_count,
        "restricted_source_enabled_count": count_true("restricted_sensitive_source_enabled"),
        "blockers": [
            "fixture runtime not implemented",
            "live access not approved",
            "CDX/Memento queries forbidden",
            "WARC/WACZ, archived page, media, transcript, newspaper page, and public document fetches forbidden",
            "scraping, crawling, browser automation, and bypass forbidden",
            "public and master index mutation forbidden",
        ],
        "readiness": "READY_FOR_H6_FIXTURE_RUNTIME",
        "truth_boundary": {
            "source_pack_is_truth": False,
            "policy_pack_grants_live_access": False,
            "web_capture_candidate_is_truth": False,
            "archived_time_state_candidate_is_truth": False,
...
def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    lines = [
        "# H6 Web Archive News Event Source Summary",
        "",
        ...
        f"- fetch_enabled_count: `{summary.get('fetch_enabled_count', 0)}`",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        raise ValueError(f"refusing output outside approved H6 summary roots: {rel}")
