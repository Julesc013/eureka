#!/usr/bin/env python3
"""Run or preflight H2 package metadata live probes with fail-closed policy gates."""
...
from control.prototypes.legacy_runtime.connectors.h2_package_registries.live_probe_common import (  # noqa: E402
    H2PackageLiveProbeBlocked,
    SOURCE_CONFIGS,
    build_h2_package_live_probe_blocked_result,
    build_h2_package_live_probe_output_bundle,
    build_h2_package_live_probe_request,
    build_h2_package_live_probe_result,
    fetch_h2_package_metadata_once,
    load_h2_package_live_probe_policy_bundle,
    summarize_h2_package_live_probe_result,
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", help="Optional explicit H2 package live-probe request envelope.")
    ...
    try:
        ...
        if not args.check:
            ...
            if args.identity_output:
                _write_json(args.identity_output, result["package_identity_candidate"])
        ...
        summary = {
            ...
            "live_probe": summarize_h2_package_live_probe_result(result),
        }
        if args.json:
            print(json.dumps(summary, indent=2, sort_keys=True), file=stdout)
        else:
            live = summary["live_probe"]
            print("H2 package metadata live probe", file=stdout)
...
def run_probe(request: Mapping[str, Any], policy_bundle: Mapping[str, Any], live: bool) -> dict[str, Any]:
    ...
    if not validation["approved"]:
        result = build_h2_package_live_probe_blocked_result(request, validation["blocked_reasons"], policy_bundle)
        return {"live_probe_result": result, "output_bundle": build_h2_package_live_probe_output_bundle(result)}
    if not live:
        result = build_h2_package_live_probe_blocked_result(request, ["dry preflight only; --live not provided"], policy_bundle)
        ...
        return {"live_probe_result": result, "output_bundle": build_h2_package_live_probe_output_bundle(result)}
    try:
        payload, metadata = fetch_h2_package_metadata_once(request, policy_bundle)
    except H2PackageLiveProbeBlocked as exc:
        result = exc.result
        return {"live_probe_result": result, "output_bundle": build_h2_package_live_probe_output_bundle(result)}
    result = build_h2_package_live_probe_result(str(request["source_id"]), payload, metadata, policy_bundle)
    return {"live_probe_result": result, "output_bundle": build_h2_package_live_probe_output_bundle(result)}


def render_summary(result: Mapping[str, Any]) -> str:
    summary = summarize_h2_package_live_probe_result(result)
    lines = [
        "# H2 Package Metadata Live Probe Summary",
        "",
...
def _load_request(args: argparse.Namespace, bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return build_h2_package_live_probe_request(args.source_id, args.request_key, bundle, live_requested=args.live and not args.check)
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h2_package_registries/live_probe_results/"):
            return resolved
        if rel_lower.startswith("examples/connectors/h2_package_registries/live_probe_outputs/"):
            return resolved
        raise ValueError(f"refusing output outside approved H2 live-probe roots: {rel}")
