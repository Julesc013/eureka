#!/usr/bin/env python3
"""Summarize H2 package fixture outputs offline."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--input", action="append", default=[], help="H2 output file or directory; repeatable.")
    ...
    try:
        inputs = args.input or ["examples/connectors/h2_package_registries"]
        ...
        if args.json:
            ...
            print("H2 package fixture output summary", file=stdout)
...
def build_summary(inputs: Sequence[str]) -> dict[str, Any]:
    ...
    normalized = [item for item in records if item.get("schema_version") == "h2_package_normalized_record.v0"]
    replay = [item for item in records if item.get("schema_version") == "h2_package_fixture_replay_result.v0"]
    identity_docs = [item for item in records if item.get("schema_version") == "h2_package_identity_candidate.v0"]
    ...
    ecosystem_counts = Counter(str(item.get("ecosystem", "unknown")) for item in normalized)
    return {
        "schema_version": "h2_package_fixture_output_summary.v0",
        "status": "pass",
        ...
        "identity_candidate_count": len(identity_docs) + sum(1 for item in normalized if item.get("package_identity_candidate")),
...
def render_summary_markdown(summary: Mapping[str, Any]) -> str:
    lines = [
        "# H2 Package Fixture Output Summary",
        "",
...
def _safe_output_path(path: Path) -> Path:
    ...
    try:
        ...
        if rel_lower.startswith("control/audits/") and "/generated/" in rel_lower:
            return resolved
        if rel_lower.startswith("examples/connectors/h2_package_registries/"):
            return resolved
