#!/usr/bin/env python3
"""Summarize H3 OS package live-probe result files without network access."""
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("H3 OS package live probe output summary", file=stdout)
...
def load_results(inputs: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for item in inputs:
        ...
        for json_path in paths:
            ...
            if isinstance(payload, dict) and payload.get("schema_version") == "h3_os_package_live_probe_result.v0":
...
def build_summary(results: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "h3_os_package_live_probe_output_summary.v0",
        "status": "pass",
        ...
        "identity_candidate_count": sum(1 for item in results if isinstance(item.get("os_package_identity_candidate"), Mapping) and item.get("os_package_identity_candidate", {}).get("status") != "not_created_blocked_by_policy"),
        "compatibility_candidate_count": sum(1 for item in results if isinstance(item.get("os_platform_compatibility_candidate"), Mapping) and item.get("os_platform_compatibility_candidate", {}).get("status") != "not_created_blocked_by_policy"),
        "dependency_candidate_count": sum(len(item.get("dependency_candidate_preview", []) or []) for item in results),
        "file_candidate_count": sum(len(item.get("package_file_candidate_preview", []) or []) for item in results),
        ...
        "product_boundary": {"enabled_source_sync": False, "enabled_repository_index_sync": False, "enabled_downloads": False, "mutated_public_index": False, "mutated_master_index": False},
    }


def render_summary(summary: Mapping[str, Any]) -> str:
    lines = [
        "# H3 OS Package Live Probe Output Summary",
        "",
        ...
        f"- repository_index_sync_used: `{str(summary.get('repository_index_sync_used')).lower()}`",
