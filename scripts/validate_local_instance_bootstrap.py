...
def validate_queue_and_context(root: Path, errors: list[str]) -> None:
    ...
    if "current_recommended_task: LOCAL-02" not in queue:
        errors.append("queue index must point to LOCAL-02")
    if "id: LOCAL-01" not in queue or "status: completed" not in queue:
        errors.append("queue index must mark LOCAL-01 completed")
    if "id: LOCAL-02" not in queue:
    ...
    if "LOCAL-02" not in task_packet:
