...
CONTRACTS = (
    ...
    "control/schemas/previews/extraction/extraction_manifest_candidate.v0.json",
    "control/schemas/previews/extraction/extraction_candidate_effect.v0.json",
    "control/schemas/audits/extraction/extraction_safety_report.v0.json",
)
POLICIES = (
...
def validate_scripts(root: Path, errors: list[str]) -> None:
    ...
    forbidden_public = subprocess.run(
        [sys.executable, "scripts/summarize_extraction_results.py", "--input", "examples/extraction/results", "--output", "data/public_index/extraction.json"],
        cwd=root,
        capture_output=True,
        text=True,
        timeout=120,
    ...
    if forbidden_public.returncode == 0 or "refusing forbidden output root" not in forbidden_public.stdout:
        errors.append("summary script must reject data/public_index output")
