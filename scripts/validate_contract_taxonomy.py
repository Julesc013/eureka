...
def validate_root_inventory(payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    if roots.get("control/schemas", {}).get("authority_class") != "CONTROL_SCHEMA":
        errors.append("control/schemas must be classified as CONTROL_SCHEMA")
...
def validate_authority_matrix(payload: Mapping[str, Any], errors: list[str]) -> None:
    ...
    for family_id, item in families.items():
        ...
        if authority in {"PRODUCT_PUBLIC_CONTRACT", "PRODUCT_INTERNAL_CONTRACT"}:
            ...
            if canonical.startswith("control/schemas/"):
                errors.append(f"{family_id} assigns product authority to control/schemas")
...
def validate_control_schemas_decision(payload: Mapping[str, Any], errors: list[str]) -> None:
    if payload.get("decision") != "retain_as_control_schema_authority_with_migration_backlog":
        errors.append("control/schemas decision must retain as control schema authority with migration backlog")
    ...
    if payload.get("control_schemas_scope") != "control/governance schemas only":
        errors.append("control/schemas scope must be control/governance schemas only")
