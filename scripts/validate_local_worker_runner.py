...
def validate_queue_state(root: Path, errors: list[str]) -> None:
    ...
    if "current_recommended_task: LOCAL-10" not in queue:
        errors.append("queue index must point to LOCAL-10")
    if "id: LOCAL-09" not in queue or "status: completed" not in queue:
        errors.append("queue index must mark LOCAL-09 completed")
    if "id: LOCAL-10" not in queue or "status: queued" not in queue:
