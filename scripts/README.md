...
These commands are stdlib-only local dry-run checks. They add no live source
calls, connector execution, source-sync worker execution, authoritative
evidence-ledger writes, source-cache mutation, candidate/public/local/master
...
extraction, file opening, archive unpacking, arbitrary local path access, URL
fetching, payload execution, OCR/transcription, public-search/page/pack/connector
...
These commands are stdlib-only local dry-run checks over approved repo examples.
They perform no public-search route, response, result-card, or order change;
hidden score; result suppression; model call; AI reranking; telemetry;
popularity, user-profile, or ad signal; source-cache or evidence-ledger
