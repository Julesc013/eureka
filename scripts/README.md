...
- `demo_cli_workbench.py`: exposes the same bootstrap exact-resolution, deterministic search, bounded subject/state listing, side-by-side comparison, manifest export, bundle export, bundle inspection, and local stored-export capabilities through the first stdlib-only native CLI surface, staying on the public side of the architecture without committing to a final CLI or TUI stack
- `demo_http_api.py`: fetches the first local stdlib machine-readable HTTP API slice over the same transport-neutral public boundary already reused by the HTML and CLI surfaces, including the bounded `/api/compare` and `/api/states` seams, returning JSON or ZIP payloads and self-hosting a temporary local stdlib server when `--base-url` is omitted
...
Deterministic tests and demo flows now cover a bounded mixed corpus composed from governed synthetic fixtures plus small recorded GitHub Releases fixtures. They also surface the first bounded evidence-summary seam, the first bounded comparison/disagreement seam, and the first bounded object/state timeline seam without implying a final provenance, trust, merge, or object-identity architecture. Live GitHub acquisition remains intentionally deferred.
