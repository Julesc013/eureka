...
- `demo_http_api.py`: fetches the first local stdlib machine-readable HTTP API slice over the same transport-neutral public boundary already reused by the HTML and CLI surfaces, returning JSON or ZIP payloads and self-hosting a temporary local stdlib server when `--base-url` is omitted
