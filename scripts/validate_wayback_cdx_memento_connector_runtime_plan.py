#!/usr/bin/env python3
"""Validate Wayback/CDX/Memento Connector Runtime Planning v0 artifacts."""
...
AUDIT_DIR = REPO_ROOT / "control" / "audits" / "wayback-cdx-memento-connector-runtime-planning-v0"
REPORT_PATH = AUDIT_DIR / "wayback_cdx_memento_connector_runtime_planning_report.json"
INVENTORY_PATH = REPO_ROOT / "control" / "inventory" / "connectors" / "wayback_cdx_memento_connector_runtime_plan.json"
DOC_PATH = REPO_ROOT / "docs" / "operations" / "WAYBACK_CDX_MEMENTO_CONNECTOR_RUNTIME_PLAN.md"
...
REQUIRED_FILES = {
    ...
    "NEXT_STEPS.md",
    "wayback_cdx_memento_connector_runtime_planning_report.json",
}
...
VALID_READINESS = {
    ...
    "blocked_uri_privacy_policy_missing",
...
def validate() -> list[str]:
    ...
    if REPORT_PATH.exists():
        ...
        if not isinstance(loaded, Mapping):
            ...
            gates = loaded.get("uri_privacy_source_policy_gate_review", {})
            if not isinstance(gates, Mapping):
                errors.append("report.uri_privacy_source_policy_gate_review must be an object")
            else:
                for field in ("uri_privacy_review_required", "source_policy_review_required", "operator_approval_required"):
                    if gates.get(field) is not True:
                        errors.append(f"report.uri_privacy_source_policy_gate_review.{field} must be true")
            ...
            for phrase in ("approval", "uri privacy", "source policy", "user-agent/contact", "rate limits", "cache", "evidence attribution", "kill switch", "operator approval"):
    ...
    if INVENTORY_PATH.exists():
        ...
        if not isinstance(loaded, Mapping):
            ...
            if loaded.get("connector_id") != "wayback_cdx_memento_connector":
                errors.append("inventory.connector_id must be wayback_cdx_memento_connector")
    ...
    if AUDIT_DIR.exists():
        require_phrases(
            ...
            (
                "does not implement",
                "no live wayback/cdx/memento calls",
        ...
        require_phrases(
            AUDIT_DIR / "URI_PRIVACY_AND_SOURCE_POLICY_GATE_REVIEW.md",
            ("uri privacy", "source policy", "operator approval", "pending", "does not fabricate"),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "URI_REVIEW_AND_NORMALIZATION_PLAN.md",
            ("reviewed source record", "raw public query parameter", "private url", "credentialed url", "public-safe hash/fingerprint", "no raw private url publication"),
            errors,
        )
        require_phrases(
        ...
        require_phrases(
            ...
            ("p72 approval complete", "uri privacy policy reviewed", "source policy reviewed", "user-agent/contact configured", "rate limits approved", "cache destination", "evidence attribution", "kill switch", "operator approval"),
            errors,
        )
        require_phrases(
            AUDIT_DIR / "DO_NOT_IMPLEMENT_YET.md",
            ("no wayback/cdx/memento api calls", "no connector runtime", "no source cache writes", "no evidence ledger writes", "no public search live fanout", "no arbitrary url fetch", "no archived content fetch", "no capture replay", "no warc download", "no index/master mutation"),
            errors,
        )
    ...
    if DOC_PATH.exists():
        text = require_phrases(
            DOC_PATH,
            ("planning-only", "no runtime", "no external calls", "no arbitrary url", "no archived content", "no mutation", "public search must not call wayback/cdx/memento live"),
            errors,
            "operations doc",
        )
...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    if args.json:
        payload = {
            "status": status,
            "report_id": "wayback_cdx_memento_connector_runtime_planning_v0",
