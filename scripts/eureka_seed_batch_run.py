...
def main(argv: Sequence[str] | None = None, stdout: TextIO = sys.stdout) -> int:
    ...
    parser.add_argument("--driver-support", action="store_true", help="Run SEED-BATCH-DRIVER-SUPPORT-00.")
    ...
    selected = [args.frontier_media, args.legacy_software, args.manuals_scans, args.driver_support]
    if sum(1 for item in selected if item) != 1:
        parser.error("choose exactly one of --frontier-media, --legacy-software, --manuals-scans, or --driver-support")
    if args.driver_support:
        result = run_seed_batch_driver_support(
            fixture=True,
            write_examples=args.write_examples,
            write_inventory=args.write_inventory,
        )
    elif args.manuals_scans:
