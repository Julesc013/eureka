...
class AideLiteVerifierTests(unittest.TestCase):
    ...
    def test_explicit_allowed_paths_override_broad_product_forbids(self) -> None:
        allowed = [".aide/**", "tests/operations/**"]
        forbidden = [".env", "evals/**", "tests/**"]
        self.assertEqual(aide_lite.classify_scope_path(".aide/evals/runs/latest.json", " M", allowed, forbidden).classification, "allowed")
        self.assertEqual(aide_lite.classify_scope_path("tests/operations/test_aide.py", " M", allowed, forbidden).classification, "allowed")
        self.assertEqual(aide_lite.classify_scope_path(".env", "??", allowed, forbidden).classification, "forbidden")

    def test_scope_patterns_fall_back_to_latest_task_packet(self) -> None:
        root = self.make_repo()
        aide_lite.write_text(root / ".aide/queue/index.yaml", "schema_version: aide.queue-index.v0\nentries: []\n")
        aide_lite.write_text(
            root / aide_lite.LATEST_PACKET_PATH,
            "# Packet\n\n## ALLOWED_PATHS\n\n- `.aide/**`\n- `control/inventory/**`\n\n## FORBIDDEN_PATHS\n\n- `.env`\n",
        )
        allowed, forbidden = aide_lite.load_scope_patterns(root)
        self.assertIn(".aide/**", allowed)
        self.assertIn("control/inventory/**", allowed)
        self.assertIn(".env", forbidden)
        self.assertEqual(aide_lite.classify_scope_path(".aide/release/report.json", " M", allowed, forbidden).classification, "allowed")
